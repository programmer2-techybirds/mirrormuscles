                ...
                if(mobileDevice){
                    ...
                    if(settings.ccauth.enabled==0){
                        ...
                        authactive.forEach(function(auth) {
                            ccauthpopup += '<img onclick="window.open(\''+jqcc.cometchat.getBaseUrl()+'functions/login/signin.php?network='+auth.toLowerCase()+'\',\'socialwindow\')" src="'+jqcc.cometchat.getBaseUrl()+'layouts/mobile/images/login'+auth.toLowerCase()+'.png" class="auth_options"></img>';
                        });
                        ccauthpopup += '</div></div>';
                        $(".cometchat_ccmobiletab_redirect ,.cometchat_ccmobiletab_tabalert").live('click', function(){

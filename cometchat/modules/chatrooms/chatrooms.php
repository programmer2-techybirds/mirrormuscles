...
function getchatroomusers() {
	...
	if(!is_array($users = getCache('chatrooms_users'.$_REQUEST['currentroom'])) || ($force == 1)) {
		...
		$sql = sql_getQuery('getchatroomusers',array('chatroomid'=>$_REQUEST['currentroom'],'timestampCondition'=>$sqlPart));
		if($guestsMode && $crguestsMode){
		...
		while ($chat = sql_fetch_assoc($query)) {
			...
			$users[$chromeReorderFix.$chat['userid']] = array('id' => (int)$chat['userid'], 'n' => $chat['username'], 'a' => $avatar, 'b' => $chat['isbanned'], 'chatroomid' => $_REQUEST['currentroom'],'s' => $chat['status']);
		}
...
function checkpassword() {
	...
	if ($userid > 0) {
		$sql = sql_getQuery('getChatroomById',array('id'=>$_REQUEST['id']));
		/* hook to get group details*/
		if (function_exists('hooks_getGroupByID')) {
			$sql = hooks_getGroupByID(array('id'=>$_REQUEST['id']));
		}
		$query = sql_query($sql,array(),1);
		if($room = sql_fetch_assoc($query)){
...
function passwordBox() {
	...
	$cc_layout = '';
	$noBar = 0;
	if(!empty($_GET['cc_layout'])){
		$cc_layout = $_GET['cc_layout'];
	}
	if(!empty($_GET['noBar'])){
		$noBar = $_GET['noBar'];
	}
	...
	$css = getDynamicScriptAndLinkTags(array('type' => "module",'name' => 'chatrooms','layout' => $layout,'ext' => 'css'));
echo <<<EOD
<!DOCTYPE html>
<html>
	<head>
...
function loadChatroomPro() {
	...
	$cc_layout = '';
	if(!empty($_GET['cc_layout'])){
		$cc_layout = '&cc_layout='.$_GET['cc_layout'];
	}
...
function generateUserlistForm($action,$userlist,$title,$submittext,$id,$inviteid,$roomname){
	...
	$base_url = BASE_URL;
	$jQuerytag = getDynamicScriptAndLinkTags(array('type' => "core",'name' => 'jquery', 'ext' => 'js'));
	$jstag = getDynamicScriptAndLinkTags(array('type' => "core",'name' => 'scroll', 'ext' => 'js'));
	$css = getDynamicScriptAndLinkTags(array('type' => "module",'name' => 'chatrooms','layout' => $layout,'ext' => 'css'));
...
function getChatroomUserIDs($chatroomid) {
	$chatroomusers = array();
	$sql = sql_getQuery('getChatroomUserIDs',array('chatroomid'=>$chatroomid));
	/* hook to get group userids*/
	if (function_exists('hooks_getGroupUserIds')) {
		$sql = hooks_getGroupUserIds(array('chatroomid'=>$chatroomid));
	}
	$query = sql_query($sql,array(),1);
	if (defined('DEV_MODE') && DEV_MODE == '1') { echo sql_error($GLOBALS['dbh']); }

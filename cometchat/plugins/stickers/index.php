...
if($action == 'sendSticker') {
	...
	foreach($category as $c){
	    if($c != '.' && $c != '..' && $c != 'truefriends'){
	        array_push($categories, $c);
	    }
	}
		array_unshift($categories, "truefriends");
	$tab = '';
	$body_content = '';
	$used = array();
	...
	foreach ($categories as $key=>$value) {
		...
		$stickerstyle .= '.'.$value.'{ background: url("'.BASE_URL.'plugins/stickers/images/'.$value.'/'.$value.'_1.png") no-repeat center}';
		...
		foreach ($images as $key=>$val) {
			if($val != '.' && $val != '..'){
				...
				$stickerstyle .= '.'.$class[0].'{ background-image: url("'.BASE_URL.'plugins/stickers/images/'.$value.'/'.$val.'")}';
			}
		}

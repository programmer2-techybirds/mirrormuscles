<?php
    foreach ($trayicon as $value){
        if($value[0]=='chatrooms'){
            if(file_exists(dirname(dirname(dirname(dirname(__FILE__)))).DIRECTORY_SEPARATOR."modules".DIRECTORY_SEPARATOR."chatrooms".DIRECTORY_SEPARATOR."layouts".DIRECTORY_SEPARATOR.$theme.DIRECTORY_SEPARATOR."js".DIRECTORY_SEPARATOR.$theme.".js")){
            include_once(dirname(dirname(dirname(dirname(__FILE__)))).DIRECTORY_SEPARATOR."modules".DIRECTORY_SEPARATOR."chatrooms".DIRECTORY_SEPARATOR."layouts".DIRECTORY_SEPARATOR.$theme.DIRECTORY_SEPARATOR."js".DIRECTORY_SEPARATOR.$theme.".js");
            }
        }
    }
?>
        ...
        return {
            ...
            initialize: function(){
                ...
                if($(document).width() <= 414 ){
                    headericon = '<div id = "cometchat_smallmenu"><img src="layouts/<?php echo $theme;?>/images/menu.svg" height="24" width="24" class="smallmenuimages"/></div>';
                }else{
                ...
                $('.cometchat_module').click(function(){
                    ...
                    if(id[2] == 'block'){
                        ...
                        var content = '<iframe id="cometchat_bots_iframe" src="'+baseUrl+'extensions/bots/index.php?cc_theme=embedded" style="height:100%;border:none;"></iframe>';
            ...
            create_side_window: function(id,title,content){
                var sidewindow = '<div id="'+id+'_window" class="cometchat_windows"><div id="cometchat_windowtitlebar">'+title+'<div id="'+id+'_closewindow" class="cometchat_closewindow" ><img src="layouts/<?php echo $theme;?>/images/removewhite.svg"/></div></div><div id="'+id+'_container" class="cometchat_side_container">'+content+'</div></div>';
                return sidewindow;
            },
            ...
            recentList: function(item){
                ...
                $.each(item, function(i, chat){
                    ...
                    if(chat.grp) {
                        ...
                        /*if(chat.type == 1) {
                            roomtype = '<img src="'+baseUrl+'layouts/'+settings.theme+'/images/lock.png" />';
                        }*/
                        ...
                        if(typeof(chat.a) == "undefined" || chat.a == ''){
                            chat.a = baseUrl+'images/noavatar.png';
                        }
                        ...
                        var msgcounter = '';
                        if(jqcc.cometchat.getThemeVariable('openChatboxId').indexOf(chat.id) > -1 && typeof(jqcc.cometchat.getThemeArray('buddylistUnreadMessageCount', chat.id)) != "undefined" && jqcc.cometchat.getThemeArray('buddylistUnreadMessageCount', chat.id) != '') {
                ...
                if(document.getElementById('cometchat_recentlist')){
                    if(recentlisttemp!=''){
                        document.getElementById('cometchat_recentlist').style.display = 'block';
                        ...
                        document.getElementById('cometchat_recentlist').style.display = 'block';
                        jqcc[settings.theme].openChatTab(1, 0);
                    }
                }
                if(jqcc.cometchat.getSessionVariable('buddylist')==1){
                    $(".cometchat_userscontentavatar").find("img").each(function(){
                ...
                $('#cometchat_recentlist div.cometchat_recentchatlist').live('click', function(e){
                    jqcc.cometchat.userClick(e.target,1);
                });
            },
            ...
            loggedOut: function(){
                ...
                if(settings.ccauth.enabled=="1" && !mobileDevice){
                    ...
                    authactive.forEach(function(auth) {
                        ccauthpopup += '<div  class="auth_options '+auth.toLowerCase()+'_auth_options" onclick="window.open(\''+baseUrl+'functions/login/signin.php?network='+auth.toLowerCase()+'\',\'socialwindow\')" ><img src="'+baseUrl+'layouts/embedded/images/login'+auth.toLowerCase()+'.svg"><span>'+auth.toLowerCase()+'</span></div>';
                    });
                    ...
                    $('#cometchat').html('<div id="cometchat_loggedout_container"><div id="cometchat_loggedout"><div><img class="cometchat_loggedout_icon" src="'+baseUrl+'layouts/'+settings.theme+'/images/exclamation.png" /></div><div>'+language[8]+'</div></div></div>');
                }
                            ...
                            if(pluginslength>0){
                                for(var i = 0; i<pluginslength; i++){
                                    ...
                                    if(settings.plugins[i] == 'audiochat'){
                                        ...
                                        smilieshtml = '<div id="smileyicon" class="ccplugins cometchat_smilies" title="'+$[name].getTitle()+'" name="'+name+'" to="'+id+'" chatroommode="0" ><img src="layouts/<?php echo $theme;?>/images/smileyicon.svg"></div>';
                                    }else if(settings.plugins[i]=='filetransfer'){
                                        filetransferhtml='<img src="'+baseUrl+'layouts/'+settings.theme+'/images/attach.svg" class="ccplugins" title="'+$[name].getTitle()+'" name="'+name+'" to="'+id+'" chatroommode="0"/>';
                                    }else if(settings.plugins[i]=='voicenote'){
                                        /*voicenotehtml='<img src="'+baseUrl+'layouts/'+settings.theme+'/images/recorder.svg" class="ccplugins" title="'+$[name].getTitle()+'" name="'+name+'" to="'+id+'" chatroommode="0"/>';*/
                                    }
                                }
                            }
                        }
                        var plugin_divider = '<div id="vline"><img src="layouts/<?php echo $theme;?>/images/vline.svg"/ style="margin-top:12px;"></div>';
                        if (audiochat == '' && avchathtml == '') {
                            plugin_divider = '';
                        }
                        headerplugin = headerplugin+avchathtml+audiochat+plugin_divider+'<div  class="cometchat_user_closebox"><img src="layouts/<?php echo $theme;?>/images/remove.svg"/></div></div>';
                        var startlink = '';
                        var endlink = '';
                        if(link!=''){
                        ...
                        cometchat_user_popup.find("textarea.cometchat_textarea").focus(function(event){
                            setTimeout(function(){
                                ...
                                if(mobileDevice && textvalue != ''){
                                    var sendicon = '<div id="cometchat_send"><img src="'+baseUrl+'layouts/'+settings.theme+'/images/send.svg" class="cometchat_tabcontentsubmit" /></div>';
                                    $('#cometchat_user_'+id+'_popup').find('#cometchat_attachements').replaceWith(sendicon);
                                }else if(mobileDevice && textvalue == ''){
                                    var attachicon = '<div id="cometchat_attachements"><img src="'+baseUrl+'layouts/'+settings.theme+'/images/attach.svg" class="ccplugins" title="'+$['ccfiletransfer'].getTitle()+'" name="ccfiletransfer" to="'+id+'" chatroommode="0"/></div>';
            ...
            addMessages: function(item,silent){
                ...
                $.each(item, function(i, incoming){
                    ...
                    if(isRealtimetranslateEnabled && jqcc.cookie('cc_rttlang') && incoming.self == 0 && incoming.message.indexOf('CC^CONTROL_') == -1){
                    ...
                    if(incoming.self !=1 && incoming.old!=1 && $('#cometchat_user_'+incoming.from+'_popup').length==0 ){
                        if(settings.autoPopupChatbox==0 && incoming.self != 1){
                            jqcc[settings.theme].addPopup(incoming.from, 1, 1);
                            jqcc.cometchat.updateChatBoxState({id:incoming.from,c:1,s:0});
                        }
                        ...
                        if($("#cometchat_message_"+messagewrapperid).length>0){
                            ...
                            if(message.indexOf('avchat_webaction=initiate')!=-1){
                                jqcc.embedded.generateIncomingAvchatData(incoming,avchat_data,currenttime);
                                ...
                                if(incoming.sent > currenttime - 15){
                                    ...
                                    jqcc.embedded.removeAVchatContainer(incoming.from);
                                }
                            }
                            ...
                            var msg2 = '<div '+avchatdisplay+' class="cometchat_messagebox '+selfmessage+'"><div class="'+add_bg+'" id="cometchat_message_'+messagewrapperid+'"><span class="cometchat_chatboxmessagecontent" style="'+imagemsg_rightcss+'">'+message+'</span>'+fromavatar+'</div><span id="cometchat_chatboxseen_'+messagewrapperid+'"></span>'+sentdata+'</div></div><div style="clear:both"></div>';
                            var msg = msg1+msg2;
                        ...
                        var alreadyreceivedunreadmessages = jqcc.cometchat.getFromStorage('receivedunreadmessages');
                        if(jqcc.cometchat.getThemeVariable('openChatboxId').indexOf(incoming.from) > -1 && incoming.self != 1 && incoming.old != 1 && ((typeof(alreadyreceivedunreadmessages[incoming.from])!='undefined'&& alreadyreceivedunreadmessages[incoming.from]<messagewrapperid) || typeof(alreadyreceivedunreadmessages[incoming.from])=='undefined')){
                            jqcc[settings.theme].addPopup(incoming.from, 1, 1);
                            ...
                            dataType: 'jsonp',
                             success: function(data){
                                if(data.logoutURL){
                                    ...
                                    auth_logout.css('background','url('+baseUrl+'layouts/'+settings.theme+'/images/logout.png) no-repeat 0px 8px');
                                    logout_click();
                                    $("#cometchat_user_"+jqcc.cometchat.getThemeVariable('openChatboxId')[0]).find('.cometchat_closebox_bottom').click();
                                    jqcc.cometchat.setSessionVariable('openChatboxId', '');
            ...
            userClick: function(listing,isrecent){
                if(typeof(isrecent) != "undefined" && isrecent == 1){
                    ...
                    if(typeof id==="undefined"||$(listing).attr('id')==''){
                        id = $(listing).parents('div.cometchat_recentchatlist').attr('id');
                    }
                    id = id.substr(21);
                } else {
                    var id = $(listing).attr('id');

...
if(isset($linkToImage)) {
    ...
    $unencryptedfilename=rawurlencode($filename);
     $text = '<a class="mediamessage" pluginname="handwrite" filename="'.$filename.'" encfilename="'.$unencryptedfilename.'" mediatype="1"  href="'.$server_url.'plugins/handwrite/download.php?file='.$unencryptedfilename.'&amp;unencryptedfilename='.$filename.'" style="display:inline-block;margin-bottom:3px;margin-top:3px;max-width:100%;"><img class="cc_handwrite_image" src="'.$linkToImage.'" border="0" height="90" width="134"></a>';
    if (substr($_REQUEST['tid'],0,1) == 'c') {
        $_REQUEST['tid'] = substr($_REQUEST['tid'],1);
        sendChatroomMessage($_REQUEST['tid'],'<div class="cometchat_hw_lang" style="display:none;">'.$handwrite_language[3].'</div>'.$text,0);
    } else {

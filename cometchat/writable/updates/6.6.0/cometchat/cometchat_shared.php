...
function sql_getQuery($key, $params = array()){
	...
	$sql = $sql_queries[$key];
	$excludeEscapeforKeys = array('querystring', 'timestampCondition', 'condition', 'prepend', 'prependCondition', 'guestpart', 'originalsql', 'extra', 'sqlpart', 'insertvalues', 'limitClause', 'usertable');
	if(!empty($params)){
...
function sql_insert_id($tablename){
	global $dbms;
...
function cometchatDBConnect(){
	sql_connect();
	if($GLOBALS['cms'] != 'mssql' && $GLOBALS['cms'] != 'pgsql'){
...
function getMembershipId($userid) {
	global $integration;
	if(method_exists($integration, 'getMembershipId') && defined('ENABLE_MEMBERSHIP_LEVEL') && ENABLE_MEMBERSHIP_LEVEL == 1){
		return $GLOBALS['integration']->getMembershipId($userid);
	}
	return '';
}
...
function getMembershipDetails($membershipid) {
	global $integration;
	if(method_exists($integration, 'getMembershipDetails') && defined('ENABLE_MEMBERSHIP_LEVEL') && ENABLE_MEMBERSHIP_LEVEL == 1){
		return $GLOBALS['integration']->getMembershipDetails($membershipid);
	}
	return '';
}
...
function getEmbedCode($param = array()){
	global $integration;
	if(method_exists($integration, 'getEmbedCode')){
		return $GLOBALS['integration']->getEmbedCode($param);
	}
	return false;
}
...
function log_error($message, $logtype) {
	global $client;

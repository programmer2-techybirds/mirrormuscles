...
function index() {
    ...
    foreach ($plugins_core as $plugin => $plugininfo) {
            ...
            $pluginaction = '<a data-toggle="tooltip" title="Add in one on one" style="color:#008000;opacity: 0.2;" href="?module=features&amp;action=addplugin&amp;data='.$plugin.'&amp;ts='.$ts.'" id="oneonone_'.$plugin.'"><i class="fa fa-lg fa-user"></i></a>';
            $oneononeactive = "";
            if (in_array($plugin, $plugins)) {
                $oneononeactive = "active";
                $pluginaction = '<a data-toggle="tooltip" title="Remove from one on one" style="color:#008000;" href="javascript:void(0)" onclick="javascript:plugins_removeplugin(\''.$no.'\')" id="oneonone_'.$plugin.'"><i class="fa fa-lg fa-user"></i></a>';
            }

            $crpluginaction = '<a data-toggle="tooltip" title="Add in group" style="color:#008000;opacity: 0.2;" href="?module=features&amp;action=addchatroomplugin&amp;data='.$plugin.'&amp;ts='.$ts.'" id="group_'.$plugin.'"><i class="fa fa-lg fa-users"></i></a>';
            $cractive = '';
            if (in_array($plugin, $crplugins)) {
                $cractive = 'active';;
                $crpluginaction = '<a title="Remove from group" data-toggle="tooltip" style="color:#008000;opacity:15;" href="javascript:void(0)" onclick="javascript:plugins_removechatroomplugin(\''.$no.'\')" id="group_'.$plugin.'"><i class="fa fa-lg fa-users"></i></a>';
            }
            ...
            $plugininfo[0] = ($plugininfo[0] == 'Writeboard') ? 'Share collaborative document' :  $plugininfo[0];
            $plugininfo[0] = ($plugininfo[0] == 'Whiteboard') ? 'Share Whiteboard document' :  $plugininfo[0];
            ...
            $upgradelink = '';
            if(!is_dir(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'plugins'.DIRECTORY_SEPARATOR.$plugin) || (function_exists('checkplan') && checkplan('plugins',$plugin) != 1)) {
                $config = '';
                $upgradelink = 'style="cursor:pointer;" onclick="javascript:window.open(\'https://www.cometchat.com/upgrade\');"';
            ...
            ${$list} .= '<tr '.$upgradelink.' group="'.$cractive.'" oneonone="'.$oneononeactive.'" id="'.$no.'" d1="'.$ti.'" rel="'.$ti.'"><td>'.$plugininfo[0].'</td><td>'.$config.'</td><td align="center">'.$pluginaction.'</td><td align="center">'.$crpluginaction.'</td></tr>';
    }
/* START: Modules*/
    global $trayicon;
    ...
    if (substr($licensekey,0,10) == "COMETCHAT-") {
        ...
        foreach ($modules_core as $module => $moduleinfo) {
            ...
            if (!empty($trayicon[$module])) {
               ...
               $removemodule = '<a data-toggle="tooltip" title="Remove feature" href="javascript:void(0)" onclick="javascript:modules_removemodule(\''.$ti[0].'\',\''.$custom.'\',this)" style="color:red;"><i class="fa fa-lg fa-minus-circle"></i></a>';
                ...
                $mactive = '';
                $removemodule = '<a data-toggle="tooltip" title="Add feature" '.$modulehref.' style="color:green;"><i class="fa fa-lg fa-plus-circle"></i></a>';
            }
            $embedlink = '';
            ...
            $upgradelink = '';
            if(!is_dir(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$module) || (function_exists('checkplan') && checkplan('modules',$module) != 1)) {
                $config = $embedlink = '';
                $upgradelink = 'onclick="javascript:window.open(\'https://www.cometchat.com/upgrade\');"';
            ...
            $moduleslist .= '<tr '.$upgradelink.' type="module'.$mactive.'"  id="'.$ti[0].'" d1="'.addslashes($ti[1]).'" d2="'.$ti[2].'" d3="'.$ti[3].'" d4="'.$ti[4].'" d5="'.$ti[5].'" d6="'.$ti[6].'" d7="'.$ti[7].'" d8="'.$ti[8].'" ><td id="'.$ti[0].'_title">'.$title.'</td><td align="center">'.$embedlink.'</td><td align="center">'.$config.'</td><td align="center">'.$removemodule.'</td></tr>';
        }
/* END: Modules*/
/* START: Extensions*/
    global $extensions;
    ...
    foreach ($extensions_core as $extension => $extensioninfo) {
        ...
        $ext_action = '<a data-toggle="tooltip" title="Add feature" '.$extensionhref.' style="color:green;"><i class="fa fa-lg fa-plus-circle"></i></a>';
        $extactive = "";
        ...
        $upgradelink = '';
        if(!is_dir(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'extensions'.DIRECTORY_SEPARATOR.$extension) || (function_exists('checkplan') && checkplan('extensions',$extension) != 1)) {
            $ext_config = '';
            $upgradelink = 'onclick="javascript:window.open(\'https://www.cometchat.com/upgrade\');"';
        ...
        $extensionslist .= '<tr '.$upgradelink.' type="extensions'.$extactive.'" id="'.$ti.'" d1="'.$ti.'" rel="'.$ti.'"><td id="'.$ti.'_title">'.stripslashes($title).'</td><td></td><td>'.$ext_config.'</td><td>'.$ext_action.'</td></tr>';
    }
/* END: Extensions*/
$body = <<<EOD

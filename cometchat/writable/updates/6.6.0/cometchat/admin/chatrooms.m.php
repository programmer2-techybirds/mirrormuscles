...
function index()
{
	...
	while ($chatroom = sql_fetch_assoc($query)) {
		...
		$chatroomlist .= '<tr><td class="capitalize">'.$chatroom['name'].$status.'</td><td>'.$chatroom['id'].'</td><td class="capitalize">'.$type.'</td><td>'.$time[0].'<td><a class="red" data-toggle="tooltip" title="Delete Group" href="?module=chatrooms&amp;action=deletechatroom&amp;data='.$chatroom['id'].'&ts='.$ts.'"><i class="fa fa-lg fa-minus-circle"></i></a></td>'.$extra.'</tr>';
	}
	$errormessage = '';
	if (!$chatroomlist) {
	  	 ...
	  	 <div class="col-sm-12 col-lg-12">
		    <div class="card">
		      ...
		      <div class="card-block">
		       ...
		       <div class="card-footer">
		        <a href="javascript:void(0);" onclick="javascript:modules_configmodule('chatrooms');">Group Settings</a>
			   </div>
		       <div class="card-footer">
...
function newchatroomprocess()
{
	...
	if (!empty($chatroom) && (strlen(trim($password)) !== 0 || empty($passwordhash))) {
		$query = sql_query('admin_createGroup',array('name'=>sanitize_core($chatroom), 'lastactivity'=>getTimeStamp(), 'password'=>$passwordhash, 'type'=>$type));

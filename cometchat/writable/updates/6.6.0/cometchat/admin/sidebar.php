...
global $client, $cms, $cmswithfriends, $MsgCnt;
...
$nav = array(
  ...
  'Settings'    => array(
    ...
    'icon'      => 'icon-settings',
    'suboption' => array(
        'General' => array('type' => 'nav-item','icon' => '','id' => '','link' => '?module=settings&action=generalsettings&ts='.$ts),
        'Web'     => array('type' => 'nav-item','icon' => '','id' => '','link' => '?module=settings&ts='.$ts),
        ...
        'Cron'    => array('type' => 'nav-item','icon' => '','id' => 'cron','link' => '?module=settings&action=cron&ts='.$ts)
    ),
  ),
  'Bots'    => array(
    'type'  => 'nav-item',
  ...
  'Support'     => array(
    ...
    'icon'      => 'icon-support',
    'target'    => '_blank',
    'link'      => 'https://my.cometchat.com/tickets'
  ),
  'Logout'    => array(
    'type'      => 'nav-item',
...
if (!((defined('SWITCH_ENABLED') && SWITCH_ENABLED == 1) || (!empty($client) && in_array($cms, $cmswithfriends)))) {
    unset($nav['Settings']['suboption']['Whose online list']);
}
if(getEmbedCode()){
  $cms = ucwords($GLOBALS['cms']);
    $nav['Settings']['suboption'][$cms] = array('type' => 'nav-item','icon' => '','id' => '','link' => '?module=settings&action=cmssettings&ts='.$ts);
}
if(!empty($client)) {
  unset($nav['Settings']['suboption']['CometService']);
  unset($nav['Settings']['suboption']['Cron']);
}

$tab = '';
foreach ($nav as $key => $value) {
  $key = ($key == "Install_top") ? "Install" : $key;
  $type = $value['type'];
  $tabslug = strtolower($key);
  $tabslug = str_replace(" ","",$tabslug);
  $tabslug = str_replace("/","",$tabslug);
  $tabslug = ($tabslug == 'groups') ? 'chatrooms' : $tabslug;
  ...
  if ($type == 'nav-title') {
      ...
      $tab .='<li class="nav-item"><a '.$target.' class="nav-link" href="'.$href.'"><i class="'.$icon.'">'.$svg.'</i> '.$key.'</a></li>';
  } else {
      $icon = $value['icon'];

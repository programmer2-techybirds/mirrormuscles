				...
				success: function(data) {
					if (data) {
						...
						if ($('#monitor').children('tbody').find('tr').length == 0) {
							htmlappend = '<tr id="noRecords"><td colspan="3">No Records Found.</td></tr>';
							$("#data").prepend(htmlappend);
						}
					}
...
function language_updatelanguage(element) {
	...
	$.post('?module=language&action=editlanguageprocess&ts='+ts, {'lang_key': lang_key, 'lang_text': lang_text, 'code': code, 'type': type, 'name': name}, function(data) {
...
function language_previewlanguage(id) {
	$("#adminModellink").trigger('click');
	$("#admin-modal-title").text('View Language');
	$('.tooltip').remove();
...
function cron_auth_link(url,get,auth) {
	...
	$("#adminModellink").trigger('click');
	$('.tooltip').remove();
	$("#admin-modal-title").text('Cron URL Code');
...
function get_automatedbots(activebots) {
	$.getJSON('//app.bots.co/api-cometchat/bots?callback=?', {}, function(data) {
		if (data) {
			...
			if (html == "") {
					html += '<tr>';
					html += '<td colspan="4">No Available Bots!</td>';
					html += '</tr>';
			}

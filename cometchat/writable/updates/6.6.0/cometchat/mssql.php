...
$sql_queries['getPrevMessages'] = "select TOP {prelimit} * from cometchat where ((cometchat.[from] = '{from}' and cometchat.[to] = '{to}' and direction <>1) or ( cometchat.[from] = '{fromid}' and cometchat.[to] = '{to}' and direction <>2 )) and cometchat.direction <> 3 {condition} order by cometchat.id desc;";
$sql_queries['getChatboxData'] = "select TOP {prelimit} * from cometchat where ((cometchat.[from] = '{from}' and cometchat.[to] = '{to}' and direction <>1) or ( cometchat.[from] = '{fromid}' and cometchat.[to] = '{to}' and direction <>2 )) and cometchat.direction <> 3 order by cometchat.id desc;";
$sql_queries['getChatboxData_prependcondition'] = " and (cometchat.id < {id}) ";
$sql_queries['getChatboxData_prepend'] = "select TOP {prelimit} * from cometchat where ((cometchat.[from] = '{from}' and cometchat.[to] = '{to}' and direction <>1) or ( cometchat.[from] = '{fromid}' and cometchat.[to] = '{to}' and direction <>2 )) {prepend} and cometchat.direction <> 3 order by cometchat.id desc;";
...
$sql_queries['getChatroomData_guestpart'] = " UNION select TOP {prependCondition} cometchat_chatroommessages.id id, cometchat_chatroommessages.message, cometchat_chatroommessages.sent, '{guestnamePrefix}' + m.name from,
...
$sql_queries['announcement_datify'] = "select TOP {limit} id,announcement,time,[to] from cometchat_announcements where [to] = '-1' {extra} order by id desc";
...
$sql_queries['getJoinedGroups'] = "select cometchat_chatrooms.id from cometchat_chatrooms where cometchat_chatrooms.id IN (select cometchat_chatrooms_users.chatroomid from cometchat_chatrooms_users where cometchat_chatrooms_users.userid = '{userid}')";
$sql_queries['groups_sqlpart'] = "(select COUNT(cometchat_chatrooms_users.userid) online from cometchat_chatrooms_users where cometchat_chatrooms_users.chatroomid = cometchat_chatrooms.id and isbanned <> 1)";
$sql_queries['getGroupsData'] = "select DISTINCT cometchat_chatrooms.id, cometchat_chatrooms.name, cometchat_chatrooms.type, cometchat_chatrooms.password, cometchat_chatrooms.lastactivity, cometchat_chatrooms.invitedusers, cometchat_chatrooms.createdby, {sqlPart} online from cometchat_chatrooms order by name asc";
...
$sql_queries['groups_guestpart'] = " UNION select {limitClause} cometchat_chatroommessages.id id, CAST(cometchat_chatroommessages.message as varchar), cometchat_chatroommessages.chatroomid, cometchat_chatroommessages.sent, '{guestnamePrefix}' + m.name [from], cometchat_chatroommessages.userid fromid, m.id userid from cometchat_chatroommessages join cometchat_guests m on cometchat_chatroommessages.userid = m.id where {timestampCondition} cometchat_chatroommessages.message not like 'banned_%' and cometchat_chatroommessages.message not like 'kicked_%' and cometchat_chatroommessages.message not like 'deletemessage_%' ";
...
$sql_queries['getBlockedUsers_guestpart'] = " UNION (select distinct(m.id) [id], '{guestnamePrefix}' + m.name [name] from cometchat_block, cometchat_guests m where m.id = toid and fromid = '{fromid}')";
...
$sql_queries['groupchathistory_guestpart'] = " union (select m1.*, '{guestnamePrefix'+f.name fromu, dateadd(S, sent, '1970-01-01') from cometchat_chatroommessages m1, cometchat_guests f where f.id = m1.userid and m1.chatroomid = '{chatroomid}' and m1.message NOT LIKE 'CC^CONTROL_deletemessage_%') order by id";
$sql_queries['groupchathistory'] = "select * from ((select m1.*, f.{usertable_username} fromu, dateadd(S, sent, '1970-01-01') from cometchat_chatroommessages m1, {usertable} f where f.{usertable_userid} = m1.userid and m1.chatroomid = '{chatroomid}' and m1.message not like '%banned%' and m1.message not like '%kicked%' and m1.message not like '%deletemessage%') {guestpart}) as t";
$sql_queries['chathistory_guestpart'] = "union (select * from ((select m1.*, '{guestnamePrefix}'+f.name fromu, '{guestnamePrefix}'+t.name tou, dateadd(S, sent, '1970-01-01') from cometchat m1, cometchat_guests f, cometchat_guests t where  f.id = m1.[from] and t.id = m1.[to] and ((m1.[from] = '{userid}' and m1.[to] = '{chatroomid}' and m1.direction <> 1) or (m1.[to] = '{userid}' and m1.[from] = '{chatroomid}' and m1.direction <> 2)) and m1.direction <> 3) union (select m1.*, f.{usertable_username} fromu, '{guestnamePrefix}'+t.name tou, dateadd(S, sent, '1970-01-01') from cometchat m1, {usertable} f, cometchat_guests t where  f.{usertable_userid} = m1.[from] and t.id = m1.[to] and ((m1.[from] = '{userid}' and m1.[to] = '{chatroomid}') or (m1.[to] = '{userid}' and m1.[from] = '{chatroomid}')) and m1.direction <> 3) order by id) as t)";
$sql_queries['chathistory'] = "(select m1.*,  f.{usertable_username} fromu, t.{usertable_username} tou, dateadd(S, sent, '1970-01-01') from [cometchat] m1, {usertable} f, {usertable} t where f.{usertable_userid} = m1.[from] and t.{usertable_userid} = m1.[to] and ((m1.[from] = '{userid}' and m1.[to] = '{chatroomid}' and m1.direction <> 1) or (m1.[to] = '{userid}' and m1.[from] = '{chatroomid}' and m1.direction <> 2)) and m1.direction <> 3) {guestpart} ";
$sql_queries['chatroomviewlog_guestpart'] = "union (select m1.id, m1.userid, m1.chatroomid, m1.sent, CONVERT(VARCHAR(MAX), m1.message), m2.name chatroom, '{guestnamePrefix}' + f.name fromu from cometchat_chatroommessages m1, cometchat_chatrooms m2, cometchat_guests f where  f.id = m1.userid and m1.chatroomid=m2.id and m1.chatroomid={chatroomid} and m1.id >= {id} and m1.message not like 'CC^CONTROL_deletemessage_%')";
...
$sql_queries['getGroupId'] = "select id from cometchat_chatrooms where guid = '{guid}'";
$sql_queries['checkUserExists'] = "select userid as count from cometchat_users where {field} = '{value}'";
$sql_queries['checkGroupUserExists'] = "select userid from cometchat_chatrooms_users where chatroomid = '{chatroomid}' and userid = '{userid}' ";
...
$sql_queries['insertRecentConversation'] = "IF ( EXISTS (SELECT * FROM [cometchat_recentconversation] WHERE cometchat_recentconversation.[id] = '{insertedid}')) begin UPDATE [cometchat_recentconversation] set cometchat_recentconversation.[from] = '{userid}',cometchat_recentconversation.[to] = '{to}',cometchat_recentconversation.[message] = '{message}',cometchat_recentconversation.[id] = '{insertedid}', cometchat_recentconversation.[sent] = '{timestamp}'end else begin INSERT cometchat_recentconversation (cometchat_recentconversation.[id], cometchat_recentconversation.[from], cometchat_recentconversation.[to], cometchat_recentconversation.[message], cometchat_recentconversation.[sent], cometchat_recentconversation.[convo_id]) VALUES ('{insertedid}', '{userid}', '{to}','{message}','{timestamp}','{convo_hash}') end";
...
$sql_queries['updateLastActivity'] = "IF ( EXISTS (SELECT * FROM [cometchat_status] WHERE cometchat_status.[userid] = '{userid}')) begin UPDATE [cometchat_status] set lastactivity = '{timestamp}',lastseen = '{timestamp}' end else begin INSERT cometchat_status (userid,lastactivity,lastseen) values ('{userid}','{timestamp}','{timestamp}') end";
...
$sql_queries['setStatus'] = "IF ( EXISTS (SELECT * FROM [cometchat_status] WHERE cometchat_status.[userid] = '{userid}')) begin UPDATE [cometchat_status] set status = '{message}' end else begin INSERT cometchat_status (userid,status) values ('{userid}','{message}') end";
...
$sql_queries['cometchatSessionOpen'] = "IF ( EXISTS (SELECT * FROM [cometchat_session] WHERE cometchat_session.[session_id] = '{session_id}')) begin UPDATE [cometchat_session] set session_lastaccesstime = GETDATE() end else begin INSERT cometchat_session (session_id,session_data) values ('{session_id}','') end";

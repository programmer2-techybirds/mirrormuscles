	    	...
	    	eventer(messageEvent,function(e) {
		        if(typeof(e.data) != 'undefined' && typeof(e.data) == 'string'){
		        	if(e.data.indexOf('ccmobile_reinitializeauth')!== -1){
		                ...
		                if(controlparameters.type == "extensions" && controlparameters.name == "jabber" && controlparameters.method == "login_gtalk"){
		                ...
		                } else if(controlparameters.type == "core" && controlparameters.name == "cometchat" && controlparameters.method == "foundation"){
		                	//Open Login Modal in chat.pcs
		                	if(typeof(jqcc(document).foundation) == 'function') {
		                		location.href = "/";
		                	}
		                }
		                else if(controlparameters.type == "plugins" && controlparameters.name == "filetransfer" && controlparameters.method == "previewCometChatMedia"){ console.log("received");
		                	if(typeof(previewCometChatMedia) == 'function') { console.log("inside");
		                		previewCometChatMedia(controlparameters.params,controlparameters.params.url);
		                	}
		                }
		                	...
		                	//notifications in DM
							var options={type:"basic", title: '<?php echo $language[48];?> '+controlparameters.params.uname, message: controlparameters.params.message,contextMessage: controlparameters.params.host_name, iconUrl: controlparameters.params.icon};
				            chrome.notifications.create("id"+controlparameters.params.mid, options);
...
function loadCCPopup(url,name,properties,width,height,title,force,allowmaximize,allowresize,allowpopout,windowmode) {
	...
	if(typeof(windowmode) != "undefined" && windowmode == 1 && typeof url != "undefined" && typeof name != "undefined") {
	...
	} else if(typeof url != "undefined" && typeof name != "undefined") {
		var mobileDevice = navigator.userAgent.match(/ipad|ipod|iphone|android|windows ce|Windows Phone|blackberry|palm|symbian/i);
		...
		if(theme == 'embedded'){
			...
			height = $(window).height() - $('#cometchat_header').height() - 40;
        ...
        if(theme == 'embedded'){
			...
			if (cometchat_container.hasClass('visible')){
				...
				if(name == 'blocks'){
					if(rtl == 1){
						cometchat_container.css({right:'100%'}).animate({"right":$(document).width()-300+'px'}, "fast").addClass('visible');
					}else{
						cometchat_container.css({left:left}).animate({"left":$(document).width()-300+'px'}, "fast").addClass('visible');
					}
				}else{
		...
		if (allowpopout == 1) {
			cometchat_container.find('.cometchat_popwindow').click(function() {
				...
				var calculatedwidth = parseInt($("#cometchat_righttab").width())+parseInt(width);
				if($('#cometchat_righttab').width()!=$(window).width()){
					if($('#cometchat_leftbar').css('left') != 'auto'){
						$('#cometchat_righttab').removeAttr('style');
						$('#cometchat_leftbar').removeAttr('style');
						$("#cometchat_righttab").find(".cometchat_textarea").animate({'width':$("#cometchat_righttab").width() - 140 + 'px'},"fast");
						$('#cometchat_leftbar').animate({'left':'0px'},"fast");
					}else{
						$("#cometchat_righttab").animate({'width':calculatedwidth},"fast");
						$("#cometchat_righttab").find(".cometchat_textarea").animate({'width':$("#cometchat_righttab").width() +(width - 140) + 'px'},"fast");
					}
				}
...
var closeCCPopup = closeCCPopup || function (id,roomid) {
	...
	if(theme == 'docked') {
		...
		jqcc('#cometchat_container_'+id).animate({"bottom":"100%"}, "fast").removeClass('visible');
	} else {
		jqcc('#cometchat_container_'+id).animate({"left":"100%"}, "fast").removeClass('visible');
		jqcc('#cometchat_container_'+id).find('.cometchat_closebox').click();
	}
	jqcc("#cometchat_tooltip").css('display', 'none');
...
function loadPopupInChatbox(url,name,fromid,toid,chatroommode) {
	if(typeof url != "undefined" && typeof name != "undefined") {
		...
		if(theme == 'embedded'){
			...
			var height = $('#cometchat_righttab').find('.cometchat_tabpopup').innerHeight()/2;
			...
			url += queryStringSeparator+'basedata='+jqcc.cometchat.getBaseData()+'&embed=web&popoutmode=null';
			if(chatroommode){
				url += queryStringSeparator+'chatroommode='+'1';
				var textfieldheight = $('#cometchat_righttab').find('#cometchat_tabinputcontainer').outerHeight(true)+1;
			}else{
			...
			if(name == 'stickers' || name == 'handwrite' || name == 'smilies' || name == 'transliterate'){
				top = 'top:100%;';
				offset = $('#cometchat_righttab').offset();
			...
			var hidden = $('#cometchat_container_'+name);
			if(theme == 'embedded'){
				if($('.cometchat_windows').hasClass('visible')){
					$('.cometchat_windows').animate({'left':'100%'},"fast").removeClass('visible');
				}
				if(name == 'stickers' || name == 'handwrite' || name == 'smilies' || name == 'transliterate'){
					...
					if (!hidden.hasClass('visible')){
						$('.cometchat_message_container').css('height',$('.cometchat_message_container').height() - $('.cometchat_container').height());
			...
			if(popup.find('.cometchat_tabcontent .cometchat_chatboxpopup_'+toid).length && name!='smilies') {
				...
				if(name=='stickers'){
					...
					window.parent.$('#cometchat_user_'+toid+'_popup').find('#cometchat_plugins_openup_icon_'+toid).removeClass('cometchat_pluginsopenup_arrowrotate');
				}
...
var closeChatboxCCPopup = closeChatboxCCPopup || function (id,chatroommode) {
	...
	var height1 = (285-(height_tabcontent)+22)+'px';
...
jqcc(function(){
	...
	if(mobileDevice==null){
		var mediaoverlay = fileTransferinterval = setInterval(function () {
			overlay = '<div class="cometchat_media_overlay" style="display:none;"><div class="cometchat_media_modal"><div><div class="cometchat_media_content"><div class="cometchat_media_container"></div></div><img class="cometchat_media_download" src="'+baseUrl+'images/download.png"><img class="cometchat_close_dialog" src="'+baseUrl+'images/close.png" ></div></div></div>';
	...
	jqcc('.mediamessage').live('click',function(e){
		if(mobileDevice==null){
			...
			var params = {"file":file, "md5file":md5file, "mediatype":mediatype, "pluginname":pluginname, "url":url};
			if(((typeof(parent) != 'undefined' && parent != null && parent != self) || window.top != window.self) && jqcc.cometchat.getCcvariable().callbackfn!='desktop'){
				var controlparameters = {"type":"plugins", "name":"filetransfer", "method":"previewCometChatMedia", "params":params};
				controlparameters = JSON.stringify(controlparameters);
				parent.postMessage('CC^CONTROL_'+controlparameters,'*');
			} else {
				previewCometChatMedia(params,url);
			}
		} else {
	...
	jqcc('.cometchat_botimagefile').live('click',function(e){
		if(mobileDevice==null){
			...
			if(((typeof(parent) != 'undefined' && parent != null && parent != self) || window.top != window.self) && jqcc.cometchat.getCcvariable().callbackfn!='desktop'){
				parent.previewCometChatMedia(controlparameters,src);
			} else {

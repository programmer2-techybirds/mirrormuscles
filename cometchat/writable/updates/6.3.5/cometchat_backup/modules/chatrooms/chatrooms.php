...
function heartbeat() {
	...
	if (count($joinedrooms) > 0 || (!empty($_REQUEST['callbackfn']) && $_REQUEST['callbackfn']=='mobileapp')) {
		...
		if(!empty($_REQUEST['callbackfn']) && $_REQUEST['callbackfn']=='mobileapp' && (empty($_REQUEST['appinfo']) || (!empty($_REQUEST['appinfo']) && $_REQUEST['appinfo']['v'] < '6.3.0'))){
			getchatroomusers();
		}
		$limit = $lastMessages;
		...
		if(isset($_POST['currentroom']) && $force == 1 && !empty($_SESSION['cometchat']['cometchat_chatroom_'.$_REQUEST['currentroom']]) && !(USE_COMET == 1 && COMET_CHATROOMS == 1)){
			$messages = getChatroomData($_REQUEST['currentroom']);
			...
			if((!empty($_REQUEST['callbackfn']) && $_REQUEST['callbackfn']=='mobileapp' && empty($_REQUEST['v'])) || (!empty($joinedrooms) && empty($_REQUEST['v']))){
				$timestampCondition = "";
				if (isset($_POST['timestamp']) && $_REQUEST['timestamp'] != 0) {
					$timestampCondition = " cometchat_chatroommessages.chatroomid in('".implode("','", $joinedrooms)."') and cometchat_chatroommessages.id > '".mysqli_real_escape_string($GLOBALS['dbh'],$_REQUEST['timestamp'])."' and ";
					$limitClause = "";
				} else {
			...
			if(mysqli_num_rows($query) > 0) {
				while ($chat = mysqli_fetch_assoc($query)) {
					...
					if ($lastMessages == 0 && $_REQUEST['timestamp'] == 0) {
						$chat['message'] = '';
					}
...
function deletechatroom(){
	...
	if (!empty($_REQUEST['id'])) {
		...
		sendChatroomMessage($_REQUEST['id'],'CC^CONTROL_'.$controlparameters,0);
...
function invite() {
	...
	if(empty($inviteContent)) {
		$inviteContent = '<div class="lobby_noroom">'.$chatrooms_language[45].'</div>';
...
<html>
	...
	<body style="margin:0px;">
		<form method="post" action="{$base_url}modules/chatrooms/chatrooms.php?action=inviteusers&embed={$embed}&basedata={$basedata}">
			<div class="cometchat_wrapper">
				<div class="container_body {$embedcss}">
					{$inviteContent}
					<div style="clear:both"></div>
				</div>
				<div class="userinvitebutton {$embedcss}" {$invitehide}>
				...
				<script>
					$(document).ready(function(){
						...
						if(mobileDevice){
							...
							$('body').css({'overflow':'none','overflow-y':'scroll'});
						} else {
							var contentheight = (window.innerHeight - jqcc('.userinvitebutton').outerHeight())+'px';
			                if(jqcc().slimScroll){
			                    $('.container_body ').css({'height': contentheight});
			                    $('.container_body ').slimScroll({scroll: 0,height: contentheight});
			                }
						}
					});
				</script>
	...
	<head>
		...
		<script type="text/javascript">
			function closePopup(name){
				var controlparameters = {'type':'modules', 'name':'chatrooms', 'method':'closeCCPopup', 'params':{'name':'invite','roomid':'{$post_roomid}'}};
...
<html>
	...
	<body>
		<div class="container passwordBox_container">
			...
			<input style="width: 95%;margin-top: 8px;margin-left: 5px;" id="chatroomPass" type="password" name="pwd" autofocus/>
				<div style="clear:both"></div>
			</div>
...
function unban() {
	...
	if($s['count'] == ''){
		$s['count'] = '<div class="lobby_noroom">'.$chatrooms_language[44].'</div>';
	}
	echo <<<EOD
<!DOCTYPE html>
<html>
		...
		<script>
			...
			$(document).ready(function(){
				...
				if(mobileDevice){
					...
					var contentheight = (window.innerHeight - jqcc('.container_sub').outerHeight())+'px';
					if(jqcc().slimScroll){
						$('.container_body ').css({'height': contentheight});
						$('.container_body ').slimScroll({scroll: 0,height: contentheight});
					}
				...
				$('.invite_1').click(function() {
				    var checked = $( "input:checked" ).length;
				    if(checked > 0){
				    	$('.unbanuser').attr("disabled", false);
				    }else{
				    	$('.unbanuser').attr("disabled", true);
				    }
				});
			});
		</script>
	...
	<body>
		<form method="post" action="{$base_url}modules/chatrooms/chatrooms.php?action=unbanusers&embed={$embed}&basedata={$basedata}">
			<div class="cometchat_wrapper">
				<div class="container_body container_body_layout1 {$embedcss}">
					{$s['count']}
					<div style="clear:both"></div>
				</div>
				<div class="container_sub container_subbox {$embedcss}">
					<input type=submit value="Unban Users" class="unbanuser" disabled />
				</div>
				<script>
					$(document).ready(function(){
						var mobileDevice = navigator.userAgent.match(/ipad|ipod|iphone|android|windows ce|Windows Phone|blackberry|palm|symbian/i);
						if(mobileDevice){
							$('.container_body').css('height',$(window).height()-($('.container_title').outerHeight(true)+$('.container_sub').outerHeight(true)+30));
							$('body').css({'overflow':'hidden','overflow-y':'auto'});
						} else {
							var contentheight = (window.innerHeight - jqcc('.container_sub').outerHeight())+'px';
			                if(jqcc().slimScroll){
			                    $('.container_body ').slimScroll({scroll: 0});
			                }
						}

						if($(".invite_1").length == 0){
							$('.container_subbox').hide();
							$('.container_body_layout1').css({'padding-top':'10px','height':'306px'});
							if($('.container_body_layout1').parent().hasClass('slimScrollDiv')){
								$('.container_body_layout1').parent().height('316px');
							}
						}
					});
				</script>
...
<html>
	...
	<body onload="{$close}">
		<div class="cometchat_wrapper">
			<div class="container_title container_titletext {$embedcss}">{$chatrooms_language[21]}</div>
			<div class="container_body container_body_layout {$embedcss} lobby_noroom">
				{$chatrooms_language[16]}
				<div style="clear:both"></div>
			</div>
		</div>

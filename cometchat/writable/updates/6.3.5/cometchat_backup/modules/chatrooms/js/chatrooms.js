    ...
    crvariables : {themename: '<?php echo $theme;?>',
            ...
            chatroomBoxKeydown: function(event,chatboxtextarea,roomno,force) {
                ...
                if ((event.keyCode == 13 && event.shiftKey == 0) || force == 1 && !$(chatboxtextarea).hasClass('cometchat_placeholder')) {
                    ...
                    if (condition) {
                        ...
                        if($('#cometchat_container_smilies').length == 1 && this.crvariables.mobileDevice){
            ...
            sendmessageProcess: function(message, currentroom, basedata, currentroomname) {
                if (message != '') {
                    ...
                    jqcc.ajax({
                        url: this.crvariables.baseUrl+"cometchat_receive.php?action=sendmessage",
                        data: {message: message , currentroom: currentroom, basedata:basedata, currentroomname: currentroomname},
                        type: 'post',
                        cache: false,
                        timeout: 10000,
                        dataType: 'jsonp',
                            ...
                            if (data) {
                                ...
                                <?php if (USE_COMET != 1 || COMET_CHATROOMS != 1):?>
                                    if (typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].addChatroomMessage) == "function")
                                        jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].addChatroomMessage(jqcc.cometchat.getChatroomVars('myid'), message,data.id,1,Math.floor(new Date().getTime()),'0', '1', currentroom);
                                <?php endif;?>
            ...
            chatroom: function(id,name,type,invite,silent,clicked,force,minimized,unreadmsgcount) {
                ...
                if (this.crvariables.currentroom != id || force == 1) {
            ...
            chatroomHeartbeat: function(forceUpdate) {
                if(forceUpdate != "undefined"  && typeof(forceUpdate) != "undefined") {
                    if(forceUpdate.toString().indexOf('^') > -1) {
                        var crid = forceUpdate.split('^')[1];
                        forceUpdate = forceUpdate.split('^')[0];
                ...
                jqcc.cometchat.setExternalVariable('force',forceUpdate);
            ...
            getChatroomDetails: function(id,loadroom,force,callback){
                var response = '';
                if(typeof(force)=='undefined'){
                    force = 0;
                }
                if(id != '') {
                    jqcc.ajax({
                        ...
                        success: function(data) {
                            response = data;
                            if(typeof(loadroom)!='undefined' && loadroom == 1){
                                if(typeof(data) != "object"){
                                    data = JSON.parse(data);
                                }
                                jqcc.cometchat.chatroom(data.id, urlencode(data.name), data.type, data.password,0,0,force);
                            }

                            if(typeof(callback) != 'undefined' && callback != ''){
                                if(typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')][callback]) != 'undefined') {
                                    jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')][callback](data.id,data);
                                }
                            }
                        }
                    });
...
<?php } ?>
    ...
    function attachPlaceholder(){
        /*jqcc('[placeholder]').not('#cometchat_chatroom_password').focus(function() {
            var input = jqcc(this);

			...
			return "<span class=\"cometchat_ts\" "+cometchat_self_ts+" "+style+">("+time.hour+":"+time.minute+time.ap+" "+time.date+time.type+" "+time.month+")</span>";
                ...
                addChatroomMessage: function(fromid,incomingmessage,incomingid,selfadded,sent,fromname,calledfromsend,chatroomid){
                    ...
                    if(message != '' && chatroomid == jqcc.cometchat.getChatroomVars('currentroom')) {
                        ...
                        if($("#cometchat_groupmessage_"+incomingid).length > 0) {
                            ...
                            if(fromid != settings.myid) {
                                ...
                                var fromavatar = '<a id="cometchat_usersavatar_'+incomingid+'" href="javascript:void(0)" onclick="javascript:parent.jqcc.cometchat.chatWith(\''+fromid+'\');" class="cometchat_float_l"><span class="cometchat_cr_other_avatar"><img class="cometchat_userscontentavatarsmall" title="'+fromname+'" src="'+jqcc.cometchat.getThemeArray('buddylistAvatar', fromid)+'"></span></a>';
                                var usernamecontent = '';
                    ...
                    if(jqcc.cometchat.getChatroomVars('owner')|| jqcc.cometchat.getChatroomVars('isModerator') || (jqcc.cometchat.getChatroomVars('allowDelete') == 1 && fromid == settings.myid)) {
                        if($("#cometchat_messagebox_"+incomingid).find(".delete_msg").length < 1) {
                            jqcc('#cometchat_messagebox_'+incomingid).append('<span class="delete_msg cometchat_margin_left" style="'+deleteiconcss+deleteiconcssself+'" onclick="javascript:jqcc.cometchat.confirmDelete(\''+incomingid+'\');"><img class="hoverbraces" src="'+baseUrl+'modules/chatrooms/bin.svg"></span>');
                        }
                    ...
                    var currentroomid = jqcc.cometchat.getChatroomVars('currentroom');
                    if(currentroomid != chatroomid && fromid != settings.myid){
                ...
                loadRoom: function(clicked,id,minimized,unreadmsgcount) {
                    ...
                    if(embeddedchatroomid==0 || (embeddedchatroomid>0 && embeddedchatroomid==roomno)){
                        ...
                        if(jqcc.cometchat.getChatroomVars('isModerator')==1){
                        ...
                        $('.cometchat_prependMessages_container > .cometchat_prependCrMessages').text('<?php echo $chatrooms_language[74];?>');
                        ...
                        var pluginsdownhtml = '<div id="cometchat_pluginuparrow_'+roomno+'_float_list" class="cometchat_float_list cometchat_arrowdown" style="top:-400px;left:-400px;display:block;"><div class="cometchat_arrow-down"></div>';
                        var audiochat = '';
                        ...
                        if(plugins.length > 0) {
                            for (var i=0;i<plugins.length;i++) {
                                ...
                                if(settings.plugins[i] == 'audiochat'){
                                    ...
                                    smilieshtml = '<div id="smileyicon" class="ccplugins cometchat_smilies" title="'+$[name].getTitle()+'" name="'+name+'" to="'+roomno+'" chatroommode="1" ><img src="themes/<?php echo $theme;?>/images/smileyicon.svg"></div>';
                                    }else if(settings.plugins[i]=='filetransfer'){
                                    filetransferhtml='<div id="cometchat_attachements"><img src="'+baseUrl+'themes/'+calleeAPI+'/images/attach.svg" class="ccplugins" title="'+$[name].getTitle()+'" name="'+name+'" to="'+roomno+'" chatroommode="1"/></div>';
                                }else if(settings.plugins[i]=='voicenote'){
                                    /*voicenotehtml = '<div id="cometchat_recorder"><img src="'+baseUrl+'themes/'+calleeAPI+'/images/recorder.svg" class="ccplugins" title="'+$[name].getTitle()+'" name="'+name+'" to="'+roomno+'" chatroommode="1"/></div>';*/
                                }else if(typeof($[name]) == 'object') {
                                    if(name == 'ccstickers' || name == 'cchandwrite' || name == 'ccscreenshare' || name == 'ccwhiteboard' || name == 'ccwriteboard' || name == 'ccbroadcast' || name == 'cctransliterate' || name == 'ccaudiochat' || name == 'ccavchat'){
                        ...
                        pluginsuphtml += '</div>';
                        pluginsdownhtml += '</div>';
                        var headerplugin = avchathtml+audiochat+'<div class="inviteChatroomUsers" id="inviteusersicon" title="<?php echo $chatrooms_language[22]; ?>"></div><div id="vline"><img src="themes/<?php echo $theme;?>/images/vline.svg"/ style="margin-top:12px;"></div> <div  class="cometchat_user_closebox"><img src="themes/<?php echo $theme;?>/images/remove.svg"/></div>';
                        pluginshtml = pluginsuphtml+pluginsdownhtml;
                        var inputcontainer = '<div id="downplugins"><div id="cometchat_pluginuparrow_'+roomno+'" class="cometchat_pluginuparrow"><img class="cometchat_pluginuparrowimage" src="themes/'+calleeAPI+'/images/circledown.svg"/></div><textarea id="cometchat_textarea_'+roomno+'" class="cometchat_textarea" style="overflow-y: hidden;"  placeholder="<?php echo $chatrooms_language[64]; ?>"></textarea>'+smilieshtml+filetransferhtml+pluginshtml+'</div>';
                    ...
                    $('#currentroom').find('.cometchat_textarea').keyup(function(event){
                        ...
                        if(mobileDevice && textvalue != '' && $('#currentroom').find('#cometchat_send').length == 0 ){
                            var sendicon = '<div id="cometchat_send"><img src="'+baseUrl+'themes/'+calleeAPI+'/images/send.svg" class="cometchat_tabcontentsubmit" /></div>';
                            ...
                            var attachicon = '<div id="cometchat_attachements"><img src="'+baseUrl+'themes/'+calleeAPI+'/images/attach.svg" class="ccplugins" title="'+$['ccfiletransfer'].getTitle()+'" name="ccfiletransfer" to="'+roomno+'" chatroommode="0"/></div>';
                    ...
                    $('#currentroom').on('click','.cometchat_tabcontentsubmit',function(){
                        ...
                        return jqcc.embedded.chatroomBoxKeydown(event, textarea, roomno,1);
                    });
                    ...
                    $('#currentroom').find('.cometchat_textarea').blur(function(event){
                        ...
                        if(textvalue == '' && mobileDevice){
                            var attachicon = '<div id="cometchat_attachements"><img src="'+baseUrl+'themes/'+calleeAPI+'/images/attach.svg" class="ccplugins" title="'+$['ccfiletransfer'].getTitle()+'" name="ccfiletransfer" to="'+roomno+'" chatroommode="1"/></div>';
                            $('#currentroom').find('#cometchat_send').replaceWith(attachicon);
                        }
                    });
                    $('#currentroom').find('.cometchat_textarea').focus(function(event){
                        setTimeout(function(){
                            ...
                            if(mobileDevice && textvalue != ''){
                                ...
                                var attachicon = '<div id="cometchat_attachements"><img src="'+baseUrl+'themes/'+calleeAPI+'/images/attach.svg" class="ccplugins" title="'+$['ccfiletransfer'].getTitle()+'" name="ccfiletransfer" to="'+id+'" chatroommode="0"/></div>';
                                $('#currentroom').find('#cometchat_send').replaceWith(attachicon);
                    ...
                    if(extensions_array.indexOf('ads') > -1){
                        jqcc.ccads.init();
                    }
                ...
                cradjustIcons: function(width,cometchat_div,id,chatroommode){
                    ...
                    if(width < 300 && cometchat_div.find('#cometchat_audiochaticon').length > 0){
                        ...
                        cometchat_div.find('#cometchat_pluginuparrow_'+id+'_float_list').append(content);
                    }
                    ...
                    if(width > 276 && cometchat_div.find('#cometchat_videochaticon').length == 0 && $.inArray('avchat',settings.plugins) != -1){
                        ...
                        cometchat_div.find('#cometchat_pluginuparrow_'+id+'_float_list').find('#cometchat_float_avchat').parent().remove();
                    }
                    if(width > 300 && cometchat_div.find('#cometchat_audiochaticon').length == 0 && $.inArray('audiochat',settings.plugins) != -1){
                        ...
                        cometchat_div.find('#cometchat_pluginuparrow_'+id+'_float_list').find('#cometchat_float_audiochat').parent().remove();
                    }
                ...
                loadChatroomList : function(item) {
                    ...
                    $.each(item, function(i,room) {
                        ...
                        var temp = '<div id="cometchat_chatroomlist_'+room.id+'" class="lobby_room'+selected+'" onmouseover="jqcc(this).addClass(\'cometchat_userlist_hover\');" onmouseout="jqcc(this).removeClass(\'cometchat_userlist_hover\');" onclick="javascript:jqcc.cometchat.chatroom(\''+room.id+'\',\''+urlencode(shortname)+'\',\''+room.type+'\',\''+room.i+'\',\''+room.s+'\',\'1\',\'1\');" style="display:block !important;"><span class="cometchat_chatroommsgcount" style="'+msgcountercss+'"><div class="cometchat_chatroommsgcounttext" amount="'+unreadmessagecount+'">'+unreadmessagecount+'</div></span><span class="lobby_room_3">'+roomtype+'</span><span class="lobby_room_4" title="<?php echo $chatrooms_language[58];?>" onclick="javascript:jqcc.cometchat.deleteChatroom(event,\''+room.id+'\');">'+deleteroom+'</span><span class="lobby_room_5" style="display:none;">'+roomowner+'</span><span class="lobby_room_6" title="<?php echo $chatrooms_language[80];?>" onclick="javascript:jqcc.'+[calleeAPI]+'.renameChatroom(event,\''+room.id+'\');">'+renameChatroom+'</span><div><span class="cometchat_chatroomimage"><img src="themes/<?php echo $theme;?>/images/group.svg"></span><span class="lobby_room_1" title="'+longname+'"><span class="currentroomname">'+longname+'</span><br/> <span class="lobby_room_2" '+userCountCss+' title="'+room.online+' <?php echo $chatrooms_language[34];?>"><?php echo  $chatrooms_language["participants"]; ?>:&nbsp;'+room.online+'</span></span></div></span></div>';
                    ...
                    $.each(item, function(i,incoming) {
                        ...
                        if (message != '' && incoming.chatroomid == jqcc.cometchat.getChatroomVars('currentroom')) {
                                ...
                                if($("#cometchat_groupmessage_"+incoming.id).length > 0) {
                                    ...
                                    var add_bg = ' cometchat_chatboxmessage';
                                    if((message.indexOf('<img')!=-1 && message.indexOf('src')!=-1 && message.indexOf('cometchat_smiley') == -1) || (smileycount > 0 && smileymsg == '')){
                                        if( incoming.fromid == settings.myid && typeof incoming.botid == "undefined") {
                                            add_bg_self = 'cometchat_chatboxselfmedia';
                                            add_bg = '';
                                        }else {
                                            add_bg = 'cometchat_chatboxmedia';
                                    ...
                                    if(incoming.fromid != settings.myid || (typeof incoming.botid != "undefined" && incoming.botid != 0)) {
                                ...
                                if(jqcc.cometchat.getChatroomVars('owner') || jqcc.cometchat.getChatroomVars('isModerator') || (incoming.fromid == settings.myid && jqcc.cometchat.getChatroomVars('allowDelete') == 1)) {
                                    if($("#cometchat_messagebox_"+incoming.id).find(".delete_msg").length < 1) {
                                        jqcc('#cometchat_messagebox_'+incoming.id).append('<span class="delete_msg '+marginclass+'" '+deletemessagecss  +' onclick="javascript:jqcc.cometchat.confirmDelete(\''+incoming.id+'\');"><img class="hoverbraces" src="'+baseUrl+'modules/chatrooms/bin.svg"></span>');
                                    }
                                    $(".cometchat_messagebox").live("mouseover",function() {
                            ...
                            if (message != '' && incoming.chatroomid != jqcc.cometchat.getChatroomVars('currentroom') && (typeof(receivedcrunreadmessages[incoming.chatroomid])=='undefined' || receivedcrunreadmessages[incoming.chatroomid] < incoming.id)){
                                ...
                                if(typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].addMessageCounter) == "function"  && incoming['message'].indexOf('CC^CONTROL_') == -1 && incoming.fromid != settings.myid) {
                                    if($.cookie(settings.cookiePrefix+"sound") && $.cookie(settings.cookiePrefix+"sound") == 'true'){
...
/*
                    ...
                    updateChatroomUsers: function(item,fetchedUsers) {
                        ...
                        $.each(item, function(i,user) {
                            ...
                            if(parseInt(user.b)!=1) {
                                ...
                                userstatus = jqcc.cometchat.getThemeArray('buddylistStatus', user.id);
                    ...
                    prependCrMessages:function(id,data){
                        ...
                        $.each(data, function(i, incoming){
                            ...
                            if (message != '') {
                                ...
                                if (incoming.fromid != settings.myid) {
                                    ...
                                    if (jqcc.cometchat.getChatroomVars('owner') || jqcc.cometchat.getChatroomVars('isModerator') || (incoming.fromid == settings.myid && jqcc.cometchat.getChatroomVars('allowDelete') == 1)) {
                                        deleteMessage = '<span class="delete_msg" onclick="javascript:jqcc.cometchat.confirmDelete(\''+incoming.id+'\');"><img class="hoverbraces" src="'+baseUrl+'modules/chatrooms/bin.svg"></span>';
                                    }
                                    var fromavatar = '<a id="cometchat_usersavatar_'+incoming.id+'" href="javascript:void(0)" class="cometchat_float_l"><span class="cometchat_cr_other_avatar"><img class="cometchat_userscontentavatarsmall" title="'+fromname+'" src="'+jqcc.cometchat.getThemeArray('buddylistAvatar', incoming.fromid)+'"></span></a>';
                                    var usernamecontent = '';
                                    if (mobileDevice || showUsername == '1') {
                                        usernamecontent = '<span class="cometchat_groupusername">'+fromname+':</span>';
                                    }
                                    temp += ('<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div style="clear:both;"></div><div class="cometchat_messagebox" id="cometchat_messagebox_'+incoming.id+'">'+fromavatar+'<div class="'+add_bg+'" id="cometchat_groupmessage_'+incoming.id+'"><div class="cometchat_messagearrow"></div>');
                                    ...
                                    if (jqcc.cometchat.getChatroomVars('owner') || jqcc.cometchat.getChatroomVars('isModerator') || (incoming.fromid == settings.myid && jqcc.cometchat.getChatroomVars('allowDelete') == 1)) {
                                        deleteMessage = '<span class="delete_msg cometchat_margin_left" style="'+deleteiconcssself+deleteiconcss+'" onclick="javascript:jqcc.cometchat.confirmDelete(\''+incoming.id+'\');"><img class="hoverbraces" src="'+baseUrl+'modules/chatrooms/bin.svg"></span>';
                                    }
                                    temp += ('<div class="cometchat_time cometchat_time_'+msg_date_class+' '+date_class+'" msg_format="'+msg_date_format+'">'+msg_date+'</div><div style="clear:both;"></div><div class="cometchat_messagebox cometchat_messagebox_self" id="cometchat_messagebox_'+incoming.id+'"><div class="'+add_bg+add_bg_self+'" id="cometchat_groupmessage_'+incoming.id+'"><span class="cometchat_chatboxmessagecontent" style="'+imagemsg_rightcss+'">'+message+'</span></div>'+$[calleeAPI].getTimeDisplay(ts,incoming.from,1)+''+deleteMessage+'</div><div style="clear:both;"></div>');
                                }
                            }
                        });
                        ...
                        if((count - parseInt(settings.prependLimit) < 0)){
                            $('#currentroom .cometchat_prependCrMessages').text('<?php echo $chatrooms_language[75];?>');
                    ...
                    activeChatrooms: function(item){
                        ...
                        $.each(chatroomitem, function(i,room) {
                            ...
                            temp += '<div id="cometchat_chatroomlist_'+room.id+'" class="lobby_room'+selected+'" onmouseover="jqcc(this).addClass(\'cometchat_userlist_hover\');" onmouseout="jqcc(this).removeClass(\'cometchat_userlist_hover\');" onclick="javascript:jqcc.cometchat.chatroom(\''+room.id+'\',\''+urlencode(shortname)+'\',\''+room.type+'\',\''+room.i+'\',\''+room.s+'\',\'1\',\'1\');" style="display:block !important;"><span class="lobby_room_3">'+roomtype+'</span><span class="lobby_room_4" title="<?php echo $chatrooms_language[58];?>" onclick="javascript:jqcc.cometchat.deleteChatroom(event,\''+room.id+'\');">'+deleteroom+'</span><span class="lobby_room_5" style="display:none;">'+roomowner+'</span><span class="lobby_room_6" title="<?php echo $chatrooms_language[80];?>" onclick="javascript:jqcc.'+[calleeAPI]+'.renameChatroom(event,\''+room.id+'\');">'+renameChatroom+'</span><div><span class="cometchat_chatroomimage"><img src="themes/<?php echo $theme;?>/images/group.svg"></span><span class="lobby_room_1" title="'+longname+'"><span class="currentroomname">'+longname+'</span><br/><span class="lobby_room_2" '+userCountCss+' title="'+room.online+' <?php echo $chatrooms_language[34];?>"><?php echo  $chatrooms_language["participants"]; ?>:&nbsp;'+room.online+'</span></span></div></span></div>';
                    ...
                    addMessageCounter: function(id, count, add){
                        ...
                        var key = '_'+id;
                        var cometchat_chatroommsgcount = $('.lobby_rooms').find('#cometchat_chatroomlist_'+id).find('.cometchat_chatroommsgcounttext');
                    ...
                    chatScroll: function(id){
                        ...
                        if($('#scrolltop_'+id).length == 0){
                            $("#currentroom_convo").prepend('<div id="scrolltop_'+id+'" class="cometchat_scrollup"><img src="'+baseUrl+'images/arrowtop.svg" class="cometchat_scrollimg" /></div>');
                        }
                        if($('#scrolldown_'+id).length == 0){
                            $("#currentroom_convo").append('<div id="scrolldown_'+id+'" class="cometchat_scrolldown"><img src="'+baseUrl+'images/arrowbottom.svg" class="cometchat_scrollimg" /></div>');
                        }
...
jqcc(document).ready(function(){
    ...
    jqcc('.inviteChatroomUsers').live('click',function(){
        ...
        var lang = '<?php echo $chatrooms_language[22];?>';
        var caller = 'cometchat_embedded_iframe';
...
jqcc('.cometchat_chatroomusers').live('click',function(){
        ...
        if (cometchat_container.hasClass('visible')){
            ...
            if(!$('.cometchat_windows').hasClass('visible') && !$('.cometchat_container').hasClass('visible')){
                if($("#cometchat_righttab").width()-cometchat_container.width() <= 400 && $('#cometchat_righttab').width()!=$(window).width()){
                    ...
                    if(rtl == 1){
                        $('#cometchat_righttab').css({'position':'absolute','right':'301px','width':$('#cometchat_righttab').width()});
                        ...
                        $('#cometchat_righttab').css({'position':'absolute','left':'301px','width':$('#cometchat_righttab').width()});
            ...
            if($('#cometchat_righttab').css('top') == "0px" || $('#cometchat_header').length != 1){
                ...
                cometchat_container.css({'top':'73px','height':'100%','position':'fixed'});
            }
    ...
    jqcc('.unbanChatroomUserembedded').live('click',function(){
        ...
        var url = baseurl+'modules/chatrooms/chatrooms.php?action=unban&caller='+caller+'&roomid='+roomid+'&inviteid='+roompass+'&basedata='+basedata+'&roomname='+roomname+'&cc_theme='+theme+'&popoutmode='+popoutmode+'&time='+Math.random();
        var mobileDevice = navigator.userAgent.match(/ipad|ipod|iphone|android|windows ce|Windows Phone|blackberry|palm|symbian/i);

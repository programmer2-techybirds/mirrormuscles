...
define('DB_PORT',                           "3306"									);
define('DB_USERNAME',                       'mirrorm0_wpnew'									);
define('DB_PASSWORD', 'jSQYVAF7KXJagxPT');
define('DB_NAME', 'mirrorm0_wpnew');

$table_prefix = 'wp_';									// Table prefix(if any)
...
class Integration{
	...
	function hooks_message($userid,$to,$unsanitizedmessage,$dir) {
		...
		$inbox_sync = mysqli_fetch_assoc($query);
		if($inbox_sync['option_value'] == 'true'){
			...
			if(!empty($unsanitizedmessage)){
				...
				$query = mysqli_query($GLOBALS['dbh'],$sql);
				$thread_id = mysqli_fetch_assoc($query);
				if(empty($thread_id['id'])) {
					...
					$thread_id['id'] = mysqli_real_escape_string($GLOBALS['dbh'] , $thread_id['id']);
					$sql = ("insert into `".TABLE_PREFIX."bp_messages_messages`(`thread_id`,`sender_id`, `subject`, `message`, `date_sent`) values ('".$thread_id['id']."','".$userid."','CometChat Conversation','".$unsanitizedmessage."','".$time."')");
					mysqli_query($GLOBALS['dbh'],$sql);
					...
					$subject = mysqli_real_escape_string($GLOBALS['dbh'] ,$subject['subject']);

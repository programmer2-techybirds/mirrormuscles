...
if (!empty($_REQUEST['timestamp'])){
	$temptimestamp = $_REQUEST['timestamp'];
}
if(!empty($_REQUEST['initialize'])) {
	$tempinitial = $_REQUEST['initialize'];
}

if (isset($_REQUEST['crtimestamp']) && isset($_REQUEST['crinitialize'])) {
	$_REQUEST['timestamp'] = $_REQUEST['crtimestamp'];
	$_REQUEST['initialize'] = $_REQUEST['crinitialize'];
}

if(!empty($trayicon['chatrooms'])) {
...
$_REQUEST['timestamp'] = $temptimestamp;
$_REQUEST['initialize'] = $tempinitial;
...
if ($userid > 0) {
    ...
    if($usebots == 1) {
    	...
    	if(((empty($_REQUEST['botlh'])) || (!empty($_REQUEST['botlh']) && $botlh != $_REQUEST['botlh'])) && !empty($botList)){

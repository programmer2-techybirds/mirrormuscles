    ...
    $.ccdocked = (function(){
        ...
        return {
            ...
            initialize: function(){
                ...
                var botslist = '<div class="cometchat_chatlisttext">'+language['no_bots']+'</div>';
                var cometchat_tabWidth = '';
                var tabcontent = '';
                var newchatoption = '';
                var cometchat_botslist = '';

                if(hasBroadcastMessage || jqcc.cometchat.getChatroomVars('allowUsers') == 1){
                    newchatoption = '<div id="cometchat_newchat" class="cometchat_tabicons"></div>';
                }
                if(settings.showOnlineTab==1){
                    tabcontent += '<div id="cometchat_contactstab" class="cometchat_tab cometchat_tab_clicked" unselectable="on"><span id="cometchat_chatstab_text" class="cometchat_tabstext">'+language['contacts']+'</span></div>';
                    tabcontent += '<div id="cometchat_groupstab" class="cometchat_tab" unselectable="on"><span id="cometchat_groupstab_text" class="cometchat_tabstext">'+language['groups']+'</span></div>';
                    ...
                    if(settings.usebots == 1) {
                        tabcontent += '<div id="cometchat_botstab" class="cometchat_tab" unselectable="on"><div class="cometchat_bottabicon cometchat_bottabstext">'+botoutline+'</div></div>';

                        cometchat_botslist = '<div id="cometchat_botslist" class="cometchat_tabhidden"><div id="cometchat_botslist_content">'+botslist+'</div></div>';
                    }
                    usertabpop = '<div id="cometchat_userstab_popup" class="cometchat_tabpopup cometchat_tabhidden"><div class="cometchat_userstabtitle"><div class="cometchat_userstabtitletext cometchat_tabtitle_header">'+language[9]+'</div><div class="cometchat_closebox cometchat_tooltip" id="cometchat_minimize_userstabpopup" title="'+language[62]+'"></div><div class="cometchat_vline"></div>'+newchatoption+announcementmodule+'<div id="cometchat_moreicon" class="cometchat_tabicons"></div></div><div id="cometchat_tabcontainer">'+tabcontent+'</div>'+findUser+'<div class="cometchat_tabcontent cometchat_tabstyle"><div id="cometchat_userscontent"><div id="cometchat_contactslist" class="cometchat_tabopen"><div id="cometchat_userslist_content">'+buddylist+'</div></div><div id="cometchat_groupslist" class="cometchat_tabhidden"><div id="cometchat_groupslist_content">'+groups+'</div></div>'+cometchat_botslist+'</div></div></div>';
                }
                ...
                if(settings.usebots == 1){
                    $('#cometchat_tabcontainer').find('div.cometchat_tab').width('100px');
                }
                if(hasChatroom == 1){
                ...
                jqcc('#cometchat_botstab').click(function(e){
                    jqcc[settings.theme].loadChatTab(3);
                    jqcc('#cometchat_groupstab').css({'border-right':'none'});
                    jqcc.ccdocked.bot_outline();
                    $("#cometchat_search").val('');
                    $('#cometchat_userscontent').find('div.cometchat_botlist').show();
                ...
                if(settings.usebots == 1){
                    jqcc.ccdocked.bot_glyph();
                    if($('#cometchat_userstab_popup').find('#cometchat_botslist').hasClass('cometchat_tabopen')){
                        $('#cometchat_userstab_popup').find('#cometchat_botslist').removeClass('cometchat_tabopen').addClass('cometchat_tabhidden');
                    }
                }
                });
                ...
                jqcc('#cometchat_contactstab').click(function(e){
                    ...
                    }else if($('#cometchat_userstab_popup').find('#cometchat_groupstab').length == 1){
                        $('#cometchat_userstab_popup').find('#cometchat_groupstab').css('border-right','1px solid #E6E7EA');                    }
                });
                ...
                cometchat_search.keyup(function(event){
                    ...
                    if($('#cometchat_userstab_popup').find('#cometchat_contactstab').hasClass('cometchat_tab_clicked')){
                        ...
                        if(searchString.length>0&&searchString!=language[18]){
                            $('#cometchat_userstab_popup').find('.cometchat_closeboxsearch').css('display','block');
                            cometchat_userscontent.find("div.cometchat_botlist").hide();
                            cometchat_userscontent.find('.cometchat_subsubtitle').hide();
                            var searchResult = cometchat_userscontent.find('div.cometchat_botscontentname:icontains('+searchString+')').parentsUntil("cometchat_botlist").show();
                            var matchLength = searchResult.length;
                            if(matchLength == 0){
                                if($('#cometchat_nousers_found').length == 0) {
                                    $('#cometchat_botslist').prepend('<div id="cometchat_nousers_found"><div class="chatmessage"><div class="search_nouser">'+language[118]+'</div></div></div></div>');
                                }
                            } else {
                                $('#cometchat_nousers_found').remove();
                            }
                ...
                if($("#cometchat_botstab").length != -1){
                    ...
                    function(){
                        if($("#cometchat_botstab").hasClass('cometchat_tab_clicked')){
                            jqcc.ccdocked.bot_outline();
                        }else{
                            jqcc.ccdocked.bot_glyph();
                        }
            ...
            loadChatTab: function(type){
                if (type > 3 || type < 0 || typeof(type)!="number" || typeof(type)=="undefined" || isNaN(type)) {
                    type = 1;
                }
                var tabs = ['recent','contacts','groups','bots'];
                ...
                if(settings.usebots != 1){
                    if($('#cometchat_userstab_popup').find('#cometchat_groupstab').hasClass('cometchat_tab_clicked')){
                        $('#cometchat_userstab_popup').find('.cometchat_tab_clicked').css('border-right','1px solid <?php echo $primary_color; ?>');
                    }else{
                        $('#cometchat_userstab_popup').find('#cometchat_groupstab').css('border-right','1px solid #E6E7EA');
                    }
                }
            ...
            botList: function(item) {
                ...
                $.each(item, function(i, bot){
                    if(bot.a == '' || typeof(bot.a) == "undefined"){
                        bot_avatar = baseUrl+"themes/docked/images/noavatar.png";
                    }else{
                        bot_avatar = bot.a;
                    }
            ...
            createChatboxData: function(id, name, status, message, avatar, link, isdevice, silent, tryOldMessages){
                    ...
                    if(chatboxOpened[id]!=null){
                        if(!$("#cometchat_user_"+id).hasClass('cometchat_tabclick')&&silent!=1){
                            ...
                            } else if($("#cometchat_user_"+id+"_popup").hasClass('cometchat_tabhidden')){
            ...
            createUnseenUser: function(flag) {
                ...
                $('#cometchat_chatbox_left').css('right',bubble_align);
            ...
            joinChatroom: function(roomid, inviteid, roomname){
                jqcc.cometchat.silentroom(roomid, inviteid, roomname);
            },
            ...
            authLogout: function(){
                auth_logout = jqcc('#cometchat_authlogout');
                jqcc.ajax({
                    ...
                    success: function(data){
                        if(data.logoutURL){
                            ...
                            $('body').append(jqcc[settings.theme].authLogin());
                            jqcc.cometchat.setThemeVariable('loggedout', 1);

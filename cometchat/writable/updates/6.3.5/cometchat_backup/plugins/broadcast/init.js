		   ...
		   return {
				...
				init: function (params) {
					if(isWindowOpen() || jqcc('#cometchat_container_'+name).length > 0) {
						alert("<?php echo $broadcast_language['popup_already_open'];?>");
						return;
					}
					var id = params.to;
				...
				join: function (params) {
					...
					if((location.protocol === 'http:' && type == '0') || mobileDevice) {
						windowMode = 1;
					}
					if(typeof(parent) != 'undefined' && parent != null && parent != self){
						var controlparameters = {"type":"plugins", "name":"core", "method":"loadCCPopup", "params":{"url": baseUrl+'plugins/broadcast/index.php?action=call&broadcast=1&chatroommode='+chatroommode+'&type=0&join=1&grp='+id+'&basedata='+basedata, "name":"broadcast", "properties":"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $width;?>,height=<?php echo $height;?>", "width":"<?php echo $width;?>", "height":"<?php echo $height;?>", "title":'<?php echo $broadcast_language[8];?>', "force":force, "allowmaximize":"1", "allowresize":allowresize, "allowpopout":"1", "windowMode":windowMode}};
	                    controlparameters = JSON.stringify(controlparameters);
	                    parent.postMessage('CC^CONTROL_'+controlparameters,'*');
					} else {
						loadCCPopup(baseUrl+'plugins/broadcast/index.php?action=call&broadcast=1&chatroommode='+chatroommode+'&type=0&join=1&grp='+id+'&basedata='+basedata, 'broadcast',"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $width;?>,height=<?php echo $height;?>",<?php echo $width;?>,<?php echo $height;?>,'<?php echo $broadcast_language[8];?>',force,1,allowresize,1,windowMode);
					}
				},

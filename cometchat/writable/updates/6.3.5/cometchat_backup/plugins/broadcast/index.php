...
if($videoPluginType == '0'){
	if (!empty($chatroommode)) {
		...
		if (empty($_REQUEST['join'])) {
			sendChatroomMessage($grp,$broadcast_language[17]." <a href='javascript:void(0);' grp='".$grp."' class='join_Broadcast' mobileAction=\"javascript:jqcc.ccbroadcast.join('".$grp."');\">".$broadcast_language[16]."</a>",0);
		}

...
if ($_REQUEST['action'] == 'request') {
	...
	$response['push']=pushMobileNotification($_REQUEST['to'],$response['id'],$processedMessage);
...
EOD;
		...
		if (!empty($_REQUEST['chatroommode'])) {
			...
			if(!empty($_REQUEST['random'])){
				sendMessage($_REQUEST['id'],$whiteboard_language[5],2);
				incrementCallback();
				$response = sendMessage($_REQUEST['id'],$whiteboard_language[2]." <a href='javascript:void(0);' class='accept_White' to='".$userid."' random='".$_REQUEST['random']."' room='".$room."' chatroommode='0' mobileAction=\"javascript:jqcc.ccwhiteboard.accept('".$userid."','".$_REQUEST['random']."','".$room."');\">".$whiteboard_language[3]."</a> ".$whiteboard_language[4],1);
				$processedMessage = $_SESSION['cometchat']['user']['n'].": ".$whiteboard_language[2];
				$response['push']=pushMobileNotification($_REQUEST['id'],$response['id'],$processedMessage);
			}
		}

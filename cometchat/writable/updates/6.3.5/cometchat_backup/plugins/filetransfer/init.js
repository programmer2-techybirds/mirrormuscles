...
(function($){
	$.ccfiletransfer = (function() {
		...
		var request = {};
		var count = 0;
		var calleeAPI = "<?php echo 'cc'.$theme; ?>";
		...
		return {
				...
				return function (e) {
					...
					if(enter == 1){
						if(!div.find("#cometchat_file_drag_"+id).length){
							/*div.append("<div class='cometchat_file_drag' id='cometchat_file_drag_"+id+"'><img src='"+baseUrl+"images/drop.svg' height='100' width='100'/><br/><?php echo $filetransfer_language['dropfiles']; ?></div>");*/
							div.append("<div class='cometchat_file_drag' id='cometchat_file_drag_"+id+"'><?php echo $filetransfer_language['dropfiles']; ?></div>");
			...
			handleResponse: function (request,id) {
				...
				if(request.readyState == 4){
				    try {
				        $("#progress_bar_"+request.count).html("<div class='progress_result'><?php echo $filetransfer_language['uploaded'];?><div>");
			...
			abortUploading: function (id) {
			  ...
			  $("#progress_bar_"+id).html('<div class="progress_result"><?php echo $filetransfer_language["upload_stopped"]; ?><div>');
			},

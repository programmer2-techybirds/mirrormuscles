...
if (empty($_GET['action'])) {
	...
	<head>
		...
		<script type="text/javascript" src="../../js.php?jsapi&type=plugin&name=transliterate"></script>
		<script>
			$ = jQuery = jqcc;
		</script>
		{$extrajs}
		...
		function onLoad() {
			...
			$("li").click(function() {
				...
				location.href = 'index.php?cc_theme={$cc_theme}&caller={$caller}&action=transliterate&basedata={$baseData}&embed={$embed}&chatroommode={$chatroommode}&id={$toId}&lang='+info;
			});

			if (jQuery().slimScroll) {
				var langheight = ($('#languages').height())+'px';

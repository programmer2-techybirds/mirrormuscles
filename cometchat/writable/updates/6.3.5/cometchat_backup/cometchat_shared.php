...
function sendChatroomMessage($to = 0,$message = '',$notsilent = 1,$messagetype='') {
	...
	$response = array("id" => $insertedid,"m" => $styleStart.$message.$styleEnd,"sent" => $timestamp);
...
function getChatroomDetails($id) {
	$response =array();
...
function getStatus() {
	...
	if(mysqli_num_rows($query) > 0) {
	    ...
	    $s = array('id' => $chat['userid'], 'n' => $chat['username'], 'l' => fetchLink($chat['link']), 'a' => getAvatar($chat['avatar']), 's' => $chat['status'], 'm' => $chat['message'],'push_channel' => 'C_'.md5($channelprefix."USER_".$userid.BASE_URL).$pushplatformsuffix, 'ccmobileauth' => $ccmobileauth, 'push_an_channel' => $announcementpushchannel.$pushplatformsuffix, 'webrtc_prefix' => $channelprefix, 'ch' => $chat['ch'], 'ls' => $chat['lastseen'], 'lstn' => $chat['lastseensetting']);

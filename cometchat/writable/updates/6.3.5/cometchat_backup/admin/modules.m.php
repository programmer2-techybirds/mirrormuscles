...
function index() {
	...
	$moduleslist = '';
        ...
        foreach ($trayicon as $trayitem => $ti) {
			...
			$src = !empty($ti[10])?$ti[10]:'../modules/'.$ti[0].'/icon.png';
			$removemodule = '<a href="javascript:void(0)" onclick="javascript:modules_removemodule(\''.$ti[0].'\',\''.$custom.'\')" style="margin-left:5px;"><img src="images/remove.png" title="Remove Module"></a>';
			$embedlink = '<a onclick="javascript:embed_link(\''.BASE_URL.''.$ti[2].'\',\''.$ti[4].'\',\''.$ti[5].'\');" href="javascript:void(0)" '.$visible.'><img src="images/embed.png" title="Generate Embed Code"></a>';
			$showtext = '<a onclick="javascript:modules_showtext(this,\''.$ti[0].'\');" href="javascript:void(0)" style="margin-left:5px;"><img src="images/text.png" style="opacity:'.$opacity.';" title="'.$showhide.' the module title in the chatbar"></a>';
			...
			$livetrayicons .= '<li class="ui-state-default" id="'.$ti[0].'" d1="'.addslashes($ti[1]).'" d2="'.$ti[2].'" d3="'.$ti[3].'" d4="'.$ti[4].'" d5="'.$ti[5].'" d6="'.$ti[6].'" d7="'.$ti[7].'" d8="'.$ti[8].'" ><img src="'.$src.'" style="margin:0;margin-top:2px;margin-right:5px;float:left;width:16px;height:16px;"></img><span style="font-size:11px;float:left;margin-top:3px;margin-left:5px;" id="'.$ti[0].'_title">'.$title.'</span><span style="font-size:11px;float:right;margin-top:0px;margin-right:5px;">'.$popup.$embedlink.' '.$config.$removemodule.'</span><div style="clear:both"></div></li>';
	}
        $errormessage = '';
        if(!$livetrayicons){
...
function createmodule() {
    ...
    if(!empty($client)) {
    	$icon = '<div class="title">Icon URL:</div><div class="element"><input type="text" class="inputbox" name="file_url"></div>';
    }else {
    	$icon = '<div class="title">Icon:</div><div class="element"><input type="file" class="inputbox" name="file"></div>';
    }
	$body = <<<EOD
	$navigation
	<script>
	...
	<div id="rightcontent" style="float:left;width:720px;border-left:1px dotted #ccc;padding-left:20px;">
		...
		<div>
			<div id="centernav">
				...
				<div class="title">Type:</div><div class="element"><select class="inputbox" name="type"><option value="">Same window<option  value="_blank">New window<option  value="_lightbox">Lightbox (same window popup)</select></div>
				<div style="clear:both;padding:10px;"></div>
...
function createmoduleprocess() {
	...
	if (empty($client) && (($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/pjpeg") || ($_FILES["file"]["type"] == "image/png"))) {
	...
	if (!empty($_POST['embeded_code'])) {
		...
		$code = "\$trayicon['".$modulename."'] = array('".$modulename."','".addslashes(addslashes(addslashes(str_replace('"','',ucfirst($_POST['title'])))))."','modules/".$modulename."/index.html','".$_POST['type']."','".$_POST['width']."','".$_POST['height']."','','1','','1','".$file_url."');";
	} else {
		$file_url = !empty($_POST['file_url'])?$_POST['file_url']:'';
		$code = "\$trayicon['".$modulename."'] = array('".$modulename."','".addslashes(addslashes(addslashes(str_replace('"','',ucfirst($_POST['title'])))))."','".$_POST['link']."','".$_POST['type']."','".$_POST['width']."','".$_POST['height']."','','','','0','".$file_url."');";
	}

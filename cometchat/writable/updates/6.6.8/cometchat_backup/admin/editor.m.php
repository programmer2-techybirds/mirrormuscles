...
if (!empty($_REQUEST['customjs']) || !empty($_REQUEST['customcss'])) {
	$enbledSettings = <<<EOD
		<form id="enablesetting" action="?module=settings&action=updatecustomsetting&ts={$ts}" method="post">
			<div class="form-group row">
            <label class="col-md-3 form-control-label">
              Enable {$filename}?
            </label>
            <label class="">
              <div style="position:relative;top:4px;"><input style="position: absolute;" type="radio" id="inline-radio1" name="{$enableoptiion}" value="1" {$ce} onclick="document.getElementById('enablesetting').submit();"></div><span style="padding-left:21px;">Yes</span>
            </label>
             <label class="">
              <div style="position:relative;"><input style="position:absolute;top:4px;left:4px;" type="radio" id="inline-radio2" name="{$enableoptiion}" value="0" {$cd} onclick="document.getElementById('enablesetting').submit();"></div><span style="padding-left:24px;">No</span>
            </label>
          </div>
        </form>
EOD;

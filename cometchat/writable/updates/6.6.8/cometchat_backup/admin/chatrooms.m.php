...
function index()
{
	...
	while ($chatroom = sql_fetch_assoc($query)) {
		...
		$chatroomlist .= '<tr><td class="capitalize">'.$chatroom['name'].'</td><td>'.$chatroom['id'].'</td><td class="capitalize">'.$type.'</td><td>'.$time[0].'<td><a class="red" data-toggle="tooltip" title="Delete Group" href="?module=chatrooms&amp;action=deletechatroom&amp;data='.$chatroom['id'].'&ts='.$ts.'"><i class="fa fa-lg fa-minus-circle"></i></a></td>'.$extra.'</tr>';
	}
	$errormessage = '';
	if (!$chatroomlist) {

...
if (empty($errormessage)) {
	...
	if (!empty($_POST['chatroommode'])) {
		if ((!empty($isImage) && $isImage) || (!empty($isVideo) && $isVideo) || (!empty($isAudio) && $isAudio) ) {
			$insertedId = sendChatroomMessage($_POST['to'],'<div style="display:none;">'.$filetransfer_language[9].'</div><br/><a class="imagemessage mediamessage" pluginname ="filetransfer" filename="'.$unencryptedfilename.'" encfilename="'.$md5filename.'" mediatype="'.$mediaType.'"  href="'.$server_url.'plugins/filetransfer/download.php?file='.$md5filename.'&amp;unencryptedfilename='.$unencryptedfilename.'" imageheight="'.$height.'" imagewidth="'.$width.'">'.$imgtag.'</a>',0);
		} else {
		...
		if ((!empty($isImage) && $isImage) || (!empty($isVideo) && $isVideo) || (!empty($isAudio) && $isAudio) ) {
			...
			$message = '<div style="display:none;">'.$filetransfer_language[5].'</div><br/><a class="imagemessage mediamessage" pluginname ="filetransfer" filename="'.$unencryptedfilename.'" encfilename="'.$md5filename.'" mediatype="'.$mediaType.'" href="'.$server_url.'plugins/filetransfer/download.php?file='.$md5filename.'&amp;unencryptedfilename='.$unencryptedfilename.'">'.$imgtag.'</a>';
		} else {
			$message = $filetransfer_language[5].' ('.$filename.'). <a class="imagemessage" href="'.$server_url.'plugins/filetransfer/download.php?file='.$md5filename.'&amp;unencryptedfilename='.$unencryptedfilename.'" target="_blank" mediatype="'.$mediaType.'">'.$filetransfer_language[6].'</a>';
		}
		$response = sendMessage($_POST['to'],$message,0,'filetransfer');
		if(USE_COMET == 1){
			$cometmessage = array();
			$cometresponse = array('to' => $_POST['to'],'message' => $message, 'dir' => 0,'type' => "filetransfer");
	...
	if (!empty($_REQUEST['callbackfn']) && $_REQUEST['callbackfn'] == 'mobileapp') {
		if (!empty($_REQUEST['callback'])) {
			echo $_REQUEST['callback'].'('.json_encode(array('id'=>$insertedId)).')';
		}else {
			echo $insertedId;
		}
		exit;

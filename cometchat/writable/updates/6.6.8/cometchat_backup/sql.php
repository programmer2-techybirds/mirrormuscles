...
$sql_queries['groupchathistory_guestpart'] = " union (select m1.*, concat('{guestnamePrefix}',f.name) fromu, from_unixtime(m1.sent,'%y,%m,%d') from cometchat_chatroommessages m1, cometchat_guests f where f.id = m1.userid and m1.chatroomid = '{chatroomid}' and m1.message NOT LIKE 'CC^CONTROL_deletemessage_%' group by date_format(from_unixtime(sent),'%y,%m,%d') desc) order by id";
$sql_queries['groupchathistory'] = "select * from ((select m1.*, f.{usertable_username} fromu, from_unixtime(m1.sent,'%y,%m,%d') from cometchat_chatroommessages m1, {usertable} f where f.{usertable_userid} = m1.userid and m1.chatroomid = '{chatroomid}' and m1.message not like '%banned%' and m1.message not like '%kicked%' and m1.message not like '%deletemessage%' group by date_format(from_unixtime(sent),'%y,%m,%d') desc) {guestpart}) as t group by date_format(from_unixtime(sent),'%y,%m,%d') desc";
$sql_queries['chathistory_guestpart'] = "union (select * from ((select m1.*, concat('{guestnamePrefix}',f.name) fromu, concat('{guestnamePrefix}',t.name) tou, from_unixtime(m1.sent,'%y,%m,%d')from cometchat m1, cometchat_guests f, cometchat_guests t where  f.id = m1.from and t.id = m1.to and ((m1.from = '{userid}' and m1.to = '{chatroomid}' and m1.direction <> 1) or (m1.to = '{userid}' and m1.from = '{chatroomid}' and m1.direction <> 2)) and m1.direction <> 3) union (select m1.*, f.{usertable_username} fromu, concat('{guestnamePrefix}',t.name) tou, from_unixtime(m1.sent,'%y,%m,%d') from cometchat m1, {usertable} f, cometchat_guests t where  f.{usertable_userid} = m1.from and t.id = m1.to and ((m1.from = '{userid}' and m1.to = '{chatroomid}') or (m1.to = '{userid}' and m1.from = '{chatroomid}')) and m1.direction <> 3) order by id) as t group by date_format(from_unixtime(sent),'%y,%m,%d') desc)";
$sql_queries['chathistory'] = "(select m1.*,  f.{usertable_username} fromu, t.{usertable_username} tou, from_unixtime(m1.sent,'%y,%m,%d') from `cometchat` m1, {usertable} f, {usertable} t where f.{usertable_userid} = m1.from and t.{usertable_userid} = m1.to and ((m1.from = '{userid}' and m1.to = '{chatroomid}' and m1.direction <> 1) or (m1.to = '{userid}' and m1.from = '{chatroomid}' and m1.direction <> 2)) and m1.direction <> 3 group by date_format(from_unixtime(sent),'%y,%m,%d') desc) {guestpart} ";
...
$sql_queries['cometchatSessionOpen'] = "insert into cometchat_session(`session_id`,`session_data`) values('{session_id}','') on duplicate key update session_lastaccesstime = NOW()";
...
$sql_queries['clearPrivateMessage'] = "delete from cometchat where (cometchat.read = 1 and ({time}-cometchat.sent)>10800) OR (({time}-cometchat.sent)>604800)";
$sql_queries['clearGuestEntries'] = "delete from cometchat_guests where id in (select userid from cometchat_status where ({time}-cometchat_status.lastactivity)>10800)";
...
class SqlQueries{
	function getQueries(){
		...
		$sqlqueries['checkUserExists'] = "select {field} as count from ".TABLE_PREFIX.DB_USERTABLE." where {field} = '{value}'";

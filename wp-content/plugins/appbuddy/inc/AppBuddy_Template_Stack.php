...
function appbuddy_login_screen() {
	...
	if( !is_user_logged_in() && AppPresser::is_app() ) {
		...
		$template = $AppBuddy->plugin['dir'] . 'templates/login.php';

		$template = apply_filters( 'appbuddy_login_screen', $template );
 ...
 * @return void
 */
function appbuddy_add_target_blank( $action, $activity = null ) {

	if( ! is_multisite() || ! isset( $activity, $activity->component ) ) return $action;
...
function appbuddy_post_modal_template() {
	...
	if ( ! function_exists('bp_activity_post_form_action') ) {
		return; // Activity Stream option is not enabled
	}

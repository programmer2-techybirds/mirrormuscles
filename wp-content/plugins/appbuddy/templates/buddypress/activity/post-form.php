<div id="activity-form-modal">
	<?php if ( function_exists( 'bp_activity_post_form_action' ) ) : ?>
		<form action="<?php bp_activity_post_form_action(); ?>" method="post" id="whats-new-form-in-modal" name="whats-new-form" role="complementary">
			...
			<p class="activity-greeting"><?php if ( bp_is_group() )
				printf( __( "What's new in %s, %s?", 'appbuddy' ), bp_get_group_name(), bp_get_user_firstname( bp_get_loggedin_user_fullname() ) );
			else
				printf( __( "What's new, %s?", 'appbuddy' ), bp_get_user_firstname( bp_get_loggedin_user_fullname() ) );
			?></p><?php echo bp_loggedin_user_domain(); ?>
	...
	<?php else:
		...
		if( current_user_can( 'manage_options' ) ) {
			$msg = __('To use the activity form you need to enable BuddyPress Activity Streams', 'appbudy');
		} else {
			$msg = __('Posting activity has been disabled', 'appbudy');
		}
	?>
		<b><?php echo $msg ?></b>
	<?php endif; ?>
		...
		<div id="activity_add_media" class="">
			<?php if(function_exists('appp_camera') && 'on' === appp_get_setting( 'appcam_appbuddy' ) ) : ?>
				<button type="button" id="attach-photo" data-nonce="<?php echo  wp_create_nonce( 'apppcamera-nonce' ); ?>" class="btn btn-primary btn-camera"><i class="fa fa-camera"></i> <?php _e('Attach Photo', 'appbudy'); ?></button>

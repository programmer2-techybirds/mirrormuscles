(function(window, document, $, undefined){
	var cookiebuster = function(){

		if( $.cookie('bp-activity-scope') )
			$.removeCookie('bp-activity-scope', { path: '/' });

		if( $.cookie('bp-activity-filter') )
		...
		jq(objects).each( function(i) {
			...
			$.removeCookie('bp-' + objects[i] + '-scope', { path: '/' });
			$.removeCookie('bp-' + objects[i] + '-filter', { path: '/' });
			$.removeCookie('bp-' + objects[i] + '-extras', { path: '/' });
	...
	$(document).on( 'load_ajax_content_done', cookiebuster );
...
jq(document).ready( function() {
	...
	/* New posts */
	jq('body').on( 'click', "#aw-whats-new-submit", function() {
		var last_date_recorded = 0;
		var button = jq(this);
		var form = button.closest("form#whats-new-form");

		jq('.ajax-spinner').show();
		...
		if ( jq("#attach-image").val() ) {
			image = '<img class="activity-image" src="' + jq("#attach-image").val() + '">';
		}

		/* Default POST values */
		var object = '';
		var item_id = jq("#whats-new-post-in").val();
		var content = jq("#whats-new").val() + image;
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				if( typeof app_buddy == 'undefined' || 
					typeof app_buddy.loggedin_user_url == 'undefined' || 
					app_buddy.loggedin_user_url == window.location.href ) {
					// only prepend activity to the user's own activity page
			...
			jq('.ajax-spinner').hide();
			jq('#activity-post-form').removeClass(UpClasses).addClass(downClasses).css('display', 'none');
	...
	jq('body').on( 'click', 'div.activity', function(event) {
		...
		if ( target.parent().hasClass('load-more') ) {
			...
			jq("#buddypress li.load-more a").html('<div class="spinner"></div>');
			...
			function(response)
			{
				...
				target.parent().hide();
				jq(document).trigger( 'load_more_ajax_done', $, jq('#main'), href );
	...
	jq('body').on( 'click', 'div.activity', function(event) {
		...
		if ( target.attr('name') == 'ac_form_submit' ) {
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				jq('.ac-input').blur(); // remove focus once the modal closes
	...
	jq('body').on( 'click', "#friend-list a.accept, #friend-list a.reject", function() {
		var button = jq(this);
	...
	jq( 'body' ).on('click', '.friendship-button a', function() {
	...
	jq('body').on('click', '.group-button .leave-group', function() {
		if ( false == confirm( BP_DTheme.leave_group_confirm ) ) {
			return false;
		}
	});

	jq('body').on('click', '#groups-dir-list .group-button a', function() {
	...
	jq('body').on( 'click', '#send_reply_button',
		function() {

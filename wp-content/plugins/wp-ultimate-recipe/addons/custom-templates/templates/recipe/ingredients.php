...
<div data-servings="<?php echo $recipe->servings_normalized(); ?>"<?php echo $this->style(); ?>>
    ...
    foreach( $groups as $index => $group ) {
        ...
        $group_class = sanitize_title( $group );
        echo '<div class="wpurp-recipe-ingredient-group-container wpurp-recipe-ingredient-group-container-' . $group_class . '">';
...
<?php
    ...
    private function ingredients_list( $recipe, $args )
    {
        ...
        foreach( $recipe->ingredients() as $ingredient ) {
            ...
            if ( !empty( $taxonomy ) && $ingredient_links != 'disabled' ) {
                ...
                if( WPURP_Taxonomy_MetaData::get( 'ingredient', $taxonomy_slug, 'hide_link' ) !== '1' ) {
                    if( $custom_link !== false && $custom_link !== '' ) {
                        if( WPURP_Taxonomy_MetaData::get( 'ingredient', $taxonomy_slug, 'nofollow_link' ) !== '1' ) {
            ...
            $out .= $plural && ($ingredient['amount_normalized'] > 1 || $ingredient['amount_normalized'] == 0 || $ingredient['unit'] != '') ? $plural : $ingredient['ingredient'];
            $out .= $closing_tag;
            $out .= '</span>';

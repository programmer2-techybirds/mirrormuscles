...
function buddyforms_process_submission( $args = array() )
{
    ...
    $the_author_id = apply_filters(
        'buddyforms_the_author_id',
        $current_user->ID,
        $form_slug,
        $post_id
    );
    ...
    foreach ( $customfields as $key => $customfield ) {
        ...
        if ( $customfield['type'] == 'taxonomy' || $customfield['type'] == 'category' || $customfield['type'] == 'tags' ) {
            ...
            if ( $customfield['taxonomy'] != 'none' && isset( $_POST[$customfield['slug']] ) ) {
                ...
                $term_list = wp_get_post_terms( $post_id, $customfield['taxonomy'], array(
                    'fields' => 'ids',
                ) );
                ...
                if ( isset( $taxonomy->hierarchical ) && $taxonomy->hierarchical == true ) {
                    $cat_string = implode( ', ', array_map( function ( $v, $k ) {
                        return sprintf( '%s', $k );
                    }, $new_tax_items, array_keys( $new_tax_items ) ) );
                } else {
                ...
                switch ( $t_field['type'] ) {
                    case 'taxonomy':
                        if ( !is_error( $post_id ) && !empty($post_id) ) {
                            $string_tmp = get_the_term_list(
                                $post_id,
                                $t_field['taxonomy'],
                                '<span class=\'' . $t_field['slug'] . '\'>',
                                ' - ',
                                '</span>'
                            );
                        }

...
class WCVendors_Pro_Shipping_Method extends WC_Shipping_Method {
	...
	public function calculate_shipping( $package = array() ) {
						...
						if ( $rate ) {
							...
							if ( empty( $vendor_shipping_cost ) ){ 
								...
								$vendor_shipping_cost = array( 
									...
									'items' 				=> array( $item_id => 
										array( 'product_id' => $product_id, 'shipping_cost' => $item_shipping_cost )
									)
								); 
								...
								$vendor_shipping_cost[ 'items' ][ $item_id ] = array( 'product_id' => $product_id, 'shipping_cost' => $item_shipping_cost ); 
    	...
    	foreach ( $package['applied_coupons'] as $coupon_code ) {
    		...
    		$coupon_id 		= ( version_compare( WC_VERSION, '2.7', '<' ) ) ? $coupon->id : $coupon->get_id(); 
    		...
    		if ( $coupon_owner == $vendor_id || in_array( 'administrator', $coupon_user->roles ) ) { 
    			...
    			$free_shipping = ( version_compare( WC_VERSION, '2.7', '<' ) ) ? $coupon->free_shipping : $coupon->get_free_shipping(); 

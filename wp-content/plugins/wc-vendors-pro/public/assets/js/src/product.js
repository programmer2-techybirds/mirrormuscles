...
jQuery( function( $ ){
	...
	*/
	...
	$('#shipping').on('click','.wcv_shipping_rates a.insert',function(){
		...
		var new_row = new_row_data.replace( 'id="_wcv_shipping_overrides_"', 'id="_wcv_shipping_overrides_' + count_row + '"' ); 
		var new_row = new_row.replace( 'name="_wcv_shipping_overrides_"', 'name="_wcv_shipping_overrides_' + count_row + '"' ); 
		$(this).closest('.wcv_shipping_rates').find('tbody').append( new_row ).find( 'select' ).select2( select2_args );

...
class WCVendors_Pro_Order_Controller {
	...
	public function table_rows( ) {
		...
		if ( !empty( $all_orders ) ) { 
			...
			foreach ( $all_orders as $_order ) { 
				...
				$new_row->order_date	= date_i18n( wc_date_format(), strtotime( $order_date ) ) . '<br /><strong>' . ucfirst( wc_get_order_status_name( $order->get_status() ) ) . '</strong>'; 
   ...
   /**
	...
	public function export_csv() {
		...
		$date_range = array( 
			'before' => date( 'Y-m-d', ( !empty( $_SESSION[ 'PV_Session' ][ '_wcv_order_end_date_input' ] ) ) 	? $_SESSION[ 'PV_Session' ][ '_wcv_order_end_date_input' ] : strtotime( apply_filters( 'wcv_order_end_date', 'now' ) )   ), 
			'after'  => date( 'Y-m-d', ( !empty( $_SESSION[ 'PV_Session' ][ '_wcv_order_start_date_input' ] ) ) 	? $_SESSION[ 'PV_Session' ][ '_wcv_order_start_date_input' ] : strtotime( apply_filters( 'wcv_order_start_date', $default_start ) )), 
		); 
		...
		$csv_rows 		= apply_filters( 'wcv_order_export_csv_rows', $csv_output->format_orders_export( WCVendors_Pro_Vendor_Controller::get_orders2( get_current_user_id(), $date_range, true ) ) );  

...
function buddyboss_media_activity_media_public_only_unfinished( $where_conditions ){
	...
	if( buddyboss_media()->option('all-media-page') && is_page( buddyboss_media()->option('all-media-page') ) ){
		...
		//$where = "( {$wpdb->prefix}bp_activity_meta.meta_key='buddyboss_media_album_id' AND"
		$args['meta_query'][] = array(
			'key'     => 'buddyboss_media_album_id',
...
function buddyboss_media_ajax_move_media(){
	...
	//Bail if new target album id is empty
	if ( empty( $new_album_id ) ) {
		$retval['message'] = __( 'Please select an album!', 'buddyboss-media' );
		die( json_encode( $retval ) );
	}
	...
	$retval = array(
		'status'	=> true,
		'message'	=> __( 'Added to album.', 'buddyboss-media' ),
	);
	die( json_encode( $retval ) );
}

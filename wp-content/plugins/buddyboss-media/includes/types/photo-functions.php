...
function bbm_total_photos_count() {
	...
	$new_sql        = "SELECT COUNT(m.id) FROM {$wpdb->prefix}buddyboss_media m INNER JOIN {$wpdb->posts} p ON p.ID = m.media_id WHERE  p.post_type = 'attachment' AND m.media_author = %d AND activity_id NOT IN ( SELECT id FROM {$wpdb->base_prefix}bp_activity WHERE is_spam = 1 )";

...
<?php
	...
	$pending_connections = pending_connection_requests('gym');
	$accepted_connections = accepted_connection_requests('gym');
	$has_not_connected = get_not_connected_friends($uid,'gym');
	$pending_parq = get_pending_client_parq($uid,'gym');
...
<?php if( $member_type=='standard' || $member_type=='pt'): ?>
<div id="buddypress">
<div id="primary">
<div class="template-my-gym">
    <div class="site-content">

    	<h3 class="template-title">My GYM</h3>
        
        <div class="col-md-12 text-right">
			<button type="button" class="btn inverse request_friend_connection">Connect Friends as GYM&nbsp;<i class="fa fa-lg fa-user-plus"></i></button>
			<p><small><i class="fa fa-info-circle"></i> The maximum number of requests to an individual is <?php echo $mm_spamer_threshold;?>, if each one is rejected you will be blocked<?php echo ( $member_type=='standard') ? '. GYM can be connected only, after you fill in trainer\'s PAR-Q.':'';?></small></p>
		</div>
		<div class="clear"></div>
		...
		<div id="connection_tabs" class="mm-tabs">
	        ...
	        <div id="connected">
				<?php if(!empty($accepted_connections)) :?>
						...
						<ul id="accepted-connections-list" class="item-list members-list" role="main">
							<?php while ( bp_members() ) : bp_the_member(); ?>
								<li>
									...
									<?php 
										$res=bp_core_get_user_domain(bp_get_member_user_id());
										$res_data=explode('/',$res);
										$result_data = $res_data[4];
									?>
	                				<div class="action col-md-5 col-xs-12 text-center">
                                        ...
                                        &nbsp;<a class="btn inverse" href="<?php echo home_url().'/vendors/'.$result_data.'-user-store/';?>">GYM Store</a>  
				                    </div>
									<div class="clear"></div>
								</li>
							<?php endwhile; ?>
					...
					<?php endif;?>
				<?php else: ?>
					<div id="message" class="info"><p>Sorry, no connected GYM were found.</p></div>
				<?php endif; ?>
		...
		<p><small><?php 
		$test = bp_get_profile_field_data('field=12&user_id='.bp_get_member_user_id());
		if(!is_array($test)) {
			echo bp_get_profile_field_data('field=12&user_id='.bp_get_member_user_id());
		} else { 
			echo implode(', ', bp_get_profile_field_data('field=12&user_id='.bp_get_member_user_id()));
		}
			...
			<?php else: ?>
				<div id="buddypress">
					<div id="message" class="info text-center">
						<p>
							All your friends are connected with you(or has pending connection) as GYM.<br>

(function(window, document, $, undefined){
	...
	app.init = function() {
		...
		if( $('body').hasClass('activity') || $('body').hasClass('group-home') ) {
			...
			// display title on iframe pages in app
		...
		app.$.body
			...
			*/
			...
			.on('click', '#main a, .single-post .entry-content p a, .activity-inner a, .external, .external a, .swiper-container, .app-root-page', function(e) {
				// need to check if url is external

				if($(e.target).hasClass('push-page')) {
					return;
				}
				...
				if( href && $(e.target).hasClass('external') || href && $( e.target ).attr('target') == '_blank' || href && !a.test(href) && href.substr(0,1) != '#' ) {
					...
					parent.postMessage( JSON.stringify(message), '*');
					return;
				} else if( href && $(e.target).hasClass('app-root-page') ) {

					// send a URL to the app to openPage(p);
